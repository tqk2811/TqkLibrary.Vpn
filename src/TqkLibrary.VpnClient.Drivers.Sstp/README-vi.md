...
| RFC 8446 / 5246 (TLS, qua `SslStream`) | `TlsByteStream` (Transport.Tls, seam `ITlsByteStream`) | [../Transport.Tls/TlsByteStream.cs:79](../TqkLibrary.VpnClient.Transport.Tls/TlsByteStream.cs#L79) | (suy luận) — BCL `SslStream.AuthenticateAsClientAsync`; net8.0 dùng overload có `CancellationToken`; cert validate qua `RemoteCertificateValidationCallback` tùy chọn (P0.6) |
