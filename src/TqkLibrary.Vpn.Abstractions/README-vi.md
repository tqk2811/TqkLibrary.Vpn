...
- **Slot L2 mới khai báo (phase L2.0, chưa hiện thực):** `INeighborResolver` (ARP/NDISC) + `IAddressConfigurator` (DHCP/SLAAC) là 2 slot của `EthernetAdapter` tương lai (design 00 §5). Dùng raw bytes/`IPAddress` (không ref `MacAddress` của project `TqkLibrary.Vpn.Ethernet`) để giữ CORE không phụ thuộc codec L2. Hiện thực ở L2.3→L2.6.
