...
├─ SoftEtherConnection.cs             Điều phối: TLS → control handshake → data channel L2 → (1-host) DHCP+ArpResolver+VirtualHost / (multiHost) ConnectUplink+MultiHostSession → facade L3 + keep-alive + reconnect; AddStationAsync thêm station
