...
{
    /// <summary>
    /// A bidirectional in-memory byte-stream pair for offline tests: bytes written to one end are readable at the other.
    /// Each direction is a byte queue guarded by its own lock + an auto-reset signal; a reader awaits the signal then
    /// drains whatever is available (partial reads, like a real socket). <see cref="CreatePair"/> returns the connected
    /// endpoints.
    /// </summary>
