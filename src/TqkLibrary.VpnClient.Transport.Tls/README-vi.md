...
| `ITlsKeyingMaterialExporter` | (Abstractions) `ExportKeyingMaterial(label, context, length)` — exporter RFC 5705 trên TLS session đã xong; chỉ `BouncyCastleTlsByteStream` hiện thực | [ITlsKeyingMaterialExporter.cs](../TqkLibrary.VpnClient.Abstractions/Transport/Interfaces/ITlsKeyingMaterialExporter.cs) |
