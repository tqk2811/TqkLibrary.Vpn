...
{
    /// <summary>
    /// Single-block DES (FIPS 46-3), ECB encrypt only, with NO weak-key checks (unlike the BCL <c>DES</c>),
    /// because MS-CHAPv2 derives DES keys from a password hash that may be weak. Cryptographically obsolete —
    /// used only for the MS-CHAPv2 challenge-response the protocol mandates.
    /// </summary>
    public static class Des
    {
        /// <summary>Encrypts one 8-byte block with an 8-byte key (parity bits are ignored, as in DES).</summary>
        public static byte[] EncryptBlock(ReadOnlySpan<byte> key8, ReadOnlySpan<byte> block8)
        {
            if (key8.Length != 8) throw new ArgumentException("key must be 8 bytes", nameof(key8));
            if (block8.Length != 8) throw new ArgumentException("block must be 8 bytes", nameof(block8));

            ulong[] subKeys = KeySchedule(ToUInt64(key8));
            ulong data = Permute(ToUInt64(block8), IP, 64);
            ...
            ulong output = Permute(preOutput, FP, 64);
            return FromUInt64(output);
        }

        static ulong[] KeySchedule(ulong key)
        {
            ulong permuted = Permute(key, PC1, 64); // 56 bits
            uint c = (uint)((permuted >> 28) & 0x0FFFFFFF);
            uint d = (uint)(permuted & 0x0FFFFFFF);
            var subKeys = new ulong[16];
            for (int i = 0; i < 16; i++)
            {
                c = RotL28(c, Shifts[i]);
                d = RotL28(d, Shifts[i]);
                ulong cd = ((ulong)c << 28) | d; // 56 bits
                subKeys[i] = Permute(cd, PC2, 56); // 48 bits
            }
            return subKeys;
        }

        static uint Feistel(uint right, ulong subKey)
        {
            ulong expanded = Permute(right, E, 32) ^ subKey; // 48 bits
            uint sboxOut = 0;
            for (int i = 0; i < 8; i++)
            {
                int six = (int)((expanded >> (42 - 6 * i)) & 0x3F);
                int row = ((six & 0x20) >> 4) | (six & 0x01);
                int col = (six >> 1) & 0x0F;
                sboxOut = (sboxOut << 4) | (uint)SBoxes[i][row * 16 + col];
            }
            return (uint)Permute(sboxOut, P, 32);
        }

        static ulong Permute(ulong input, int[] table, int inputBits)
        {
            ulong output = 0;
            for (int i = 0; i < table.Length; i++)
            {
                int bit = (int)((input >> (inputBits - table[i])) & 1);
                output = (output << 1) | (uint)bit;
            }
            return output;
        }

        static uint RotL28(uint v, int n) => ((v << n) | (v >> (28 - n))) & 0x0FFFFFFF;

        static ulong ToUInt64(ReadOnlySpan<byte> b)
        {
            ulong v = 0;
        ...
        static byte[] FromUInt64(ulong v)
        {
            var b = new byte[8];
