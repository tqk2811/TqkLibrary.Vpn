...
├─ Config/
│  ├─ N2nConfig.cs                 community + static overlay IP/prefix + edge MAC + transform (NULL/AES + key) → ToTunnelConfig() (static IP, KHÔNG DHCP)
...
| Type | Vai trò | Vị trí |
|------|---------|--------|
| `N2nDriver` | `IVpnProtocolDriver`: capabilities (**`L2Ethernet`**, **không PPP**, `None` (transform tùy chọn AES, không kind chuẩn), `Udp`, `PreSharedKey` (community), `OutOfBand` (static `-a`)); `ConnectAsync` dựng `N2nConnection` từ `N2nConfig` + endpoint | [N2nDriver.cs:18](N2nDriver.cs#L18) |
...
| `N2nConfig` | Config tĩnh: `Community`/`OverlayAddress`+`PrefixLength`/`EdgeMac`(null⇒random)/`Transform`(NULL/AES)+`AesKey`/`DnsServers`/`Routes`/`Mtu`; `ToTunnelConfig` (static IP + route mặc định = overlay subnet) | [Config/N2nConfig.cs:20](Config/N2nConfig.cs#L20) |
| `N2nEthernetChannel` | `IEthernetChannel` (`Medium=Ethernet`, `MaxHeaderLength=14`, `RequiresLinkAddressResolution=true`): `WriteFrameAsync` đọc dst-MAC từ header → `EncodePacket` (transform) → sink UDP; `Deliver` raise `InboundFrame` | [DataChannel/N2nEthernetChannel.cs:25](DataChannel/N2nEthernetChannel.cs#L25) |
...
| `IN2nTransportFactory` / `N2nTransportHandle` / `N2nSocketTransportFactory` | Seam dựng UDP transport (1 datagram = 1 message, **không framing**) + socket thật `UdpClient` + receive loop — live-only, cross-TFM (nhái Nebula) | [Transport/IN2nTransportFactory.cs:11](Transport/IN2nTransportFactory.cs#L11) |
...
- **2 bug interop sửa qua live** (self-pair offline KHÔNG bắt — đối chiếu `sn_utils.c` `update_edge`/`auth_edge`, **KHÔNG copy GPL**):
...
- **Chưa**:
  - **header encryption** (`-H`: SPECK + Pearson-256 key-derivation) — cần thêm Speck + Pearson-256 vào Crypto; validate chạy header-enc OFF.
  - **transform key-derivation** (Pearson-256 của password cho AES/ChaCha20) — hiện `N2nAesTransform` nhận key sẵn; transform ChaCha20/Speck chưa làm.
