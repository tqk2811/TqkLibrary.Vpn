...
- **Phụ thuộc (ProjectReference) — 3 project:**
  ...
  - [TqkLibrary.VpnClient.Drivers.Core](../TqkLibrary.VpnClient.Drivers.Core) — supervisor dùng chung [`ReconnectingVpnConnection<TState>`](../TqkLibrary.VpnClient.Drivers.Core/ReconnectingVpnConnection.cs#L25) + [`VpnReconnectOptions`](../TqkLibrary.VpnClient.Drivers.Core/Models/VpnReconnectOptions.cs#L17) (F.6 — gom link-loss/auto-reconnect/backoff-jitter/state/facade; `SstpConnection`/`SstpReconnectOptions` kế thừa).
...
| `SstpReconnectOptions` | **Kế thừa [`VpnReconnectOptions`](../TqkLibrary.VpnClient.Drivers.Core/Models/VpnReconnectOptions.cs#L17)** (F.6 — backoff/jitter/max-attempts/Enabled ở base); thêm **`ReadTimeout`** riêng SSTP (cap read không-tiến-triển, P1.5 — mặc định 60s, `Timeout.InfiniteTimeSpan` tắt) | [SstpReconnectOptions.cs:14](SstpReconnectOptions.cs#L14) |
...
4. `LinkUp` (IPCP đã cấp IP) ⇒ (nếu bật IPv6) `AwaitIpv6GraceAsync` chờ ngắn 2s cho IPV6CP mở để lộ link-local `fe80::/64` vào `TunnelConfig.AssignedAddressV6` (best-effort, không fail nếu server không có IPV6CP) → cắm kênh PPP vào facade `Facade` (`SwappablePacketChannel` của base) và bật keepalive — [SstpConnection.cs:237-238](SstpConnection.cs#L237-L238), [AwaitIpv6GraceAsync @ :260](SstpConnection.cs#L260).
