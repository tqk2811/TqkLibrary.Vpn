...
└── CommandModules/
    ...
    ├── CommandModuleBase.cs                  base abstract: option chung --vpn + --watermark (chỉ SoftEther) + --ipv6 + --native-esp (chỉ L2TP/IPsec native ESP proto-50, P0.8c; scheme khác ⇒ cảnh báo + bỏ qua) + parse target + header (Protocol) + connect VPN (giữ vòng đời) -> PrintCapabilitiesAsync (panel khả năng) -> RunAsync (abstract); ConnectAsync dispatch theo VpnTarget.Protocol (4 nhánh, truyền enableIpv6/useNativeEsp cho L2TP); ValidateOptions (virtual, fail-fast option riêng)
