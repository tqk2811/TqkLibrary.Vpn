...
- **netstandard2.0 vs net8.0:** ns2.0 không có `Socket.ReceiveFrom(..., CancellationToken)` ⇒ cancel/teardown unblock bằng **dispose socket** (như cầu DTLS). `WindowsIdentity`/`WindowsPrincipal` ngoài surface ns2.0 ⇒ admin-detect Windows rào `#if NET5_0_OR_GREATER` (ns2.0 dựa probe socket). `IPAddress(ReadOnlySpan<byte>)` rào `#if NET6_0_OR_GREATER` (else `.ToArray()`). `NativeRawSocket` dùng `SafeSocketHandle(IntPtr,bool)` để adopt fd libc — public ctor này **không** có trong surface ns2.0 ⇒ libc-path rào `#if !NETSTANDARD2_0`, ns2.0 fallback managed ctor (runtime ship native-ESP là net8.0).
- **Hạn chế đã biết / việc sau:**
  ...
  - **Validate live END-TO-END (2026-06-23, P0.8c, lab `l2tp-nonat/` strongSwan no-NAT, Docker bridge):** raw proto-50 socket mở OK → gửi/nhận ESP native (tcpdump proto50>0, udp4500=0) → server cài **SA native ESP** (KHÔNG `encap`/espinudp) → tunnel L2TP/PPP lên hẳn (IP `10.31.0.10`, ICMP qua tunnel RTT 3ms). **Bug transport phát hiện + sửa qua validate live: `NativeRawSocket` EPROTONOSUPPORT** — managed `Socket(Raw,proto-50)` fail trên Linux dù kernel hỗ trợ ⇒ mở fd qua libc `socket()` P/Invoke; TRƯỚC fix native-ESP/GRE 100% hỏng trên Linux (raw socket không mở được). (Hai bug còn lại của P0.8c thuộc tầng IKE/driver — honest-identity + Quick Mode encap-order — xem [README Drivers.L2tpIpsec](../TqkLibrary.VpnClient.Drivers.L2tpIpsec/README-vi.md).) Windows best-effort vẫn chờ kiểm; IPv6 receive chờ **Q.1**.
