...
namespace TqkLibrary.VpnClient.Drivers.GreInUdp.Tests
{
    ...
    /// datagram to the peer's inbound queue, preserving boundaries like a UDP socket. Stands in for the connected UDP
    /// transport so the GRE-in-UDP driver's data path can be driven offline. Throwaway test scaffolding (mirrors the
    /// IpEncap.Tests / Drivers.IpEncap.Tests helper).
    /// </summary>
