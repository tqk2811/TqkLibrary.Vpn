...
└── CommandModules/
    ...
    ├── CommandModuleBase.cs                  base GENERIC: --check-url/--proxy-host/--proxy-port + header/IP/bắt lỗi + TcpIpStack -> VpnProxySource -> ProxyServer (giữ tới khi nhấn Enter); abstract ConnectAsync(ParseResult)
...
Profile có sẵn: `sstp`, `l2tp`, `sstp (host khác)`, `l2tp (host khác)`, `sstp (proxy cố định)`, `l2tp (proxy LAN)`, `help`.
