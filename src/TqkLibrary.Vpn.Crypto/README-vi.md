...
| RFC 2759 (MS-CHAPv2) | `Md4` + `Des` (làm primitive) | [Md4.cs:6](Md4.cs#L6), [Des.cs:5-6](Des.cs#L5-L6) | (suy luận) primitive ở đây; thuật toán CHAPv2 thực thi ở project `TqkLibrary.Vpn.Ppp` |
...
- **AES-GCM split runtime** ([Aead/AesGcmCipher.cs:49-62](Aead/AesGcmCipher.cs#L49-L62) và [:74-103](Aead/AesGcmCipher.cs#L74-L103)): nhánh `#if NET8_0_OR_GREATER` dùng `System.Security.Cryptography.AesGcm`; nhánh `#else` dùng `GcmBlockCipher(new AesEngine())` của BouncyCastle. `Open` trả `false` khi tag sai (`AuthenticationTagMismatchException` trên net8.0; `InvalidCipherTextException` trên netstandard2.0) thay vì ghi plaintext. Alias type cụ thể để tránh đụng `Org.BouncyCastle...IAeadCipher` với interface cùng tên của project ([Aead/AesGcmCipher.cs:4-10](Aead/AesGcmCipher.cs#L4-L10)).
