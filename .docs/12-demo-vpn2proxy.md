...
([`TcpIpStack.PingAsync`](../src/TqkLibrary.VpnClient.IpStack/TcpIpStack.cs#L107) — [`ProbeVirtualLanAsync` @ :106](../demo/Vpn2ProxyDemo/VpnCapabilityProbe.cs#L106); **phát hiện LAN ảo thì panel thêm dòng "Gateway nội bộ"** ở phần Info); (2) **năng lực
driver tĩnh** đọc thẳng từ `Capabilities` của driver tương ứng ([`SstpDriver`](../src/TqkLibrary.VpnClient.Drivers.Sstp/SstpDriver.cs#L37)/[`L2tpIpsecDriver`](../src/TqkLibrary.VpnClient.Drivers.L2tpIpsec/L2tpIpsecDriver.cs#L40)/[`SoftEtherDriver`](../src/TqkLibrary.VpnClient.Drivers.SoftEther/SoftEtherDriver.cs#L74)/[`OpenVpnDriver`](../src/TqkLibrary.VpnClient.Drivers.OpenVpn/OpenVpnDriver.cs#L72))
...
stream duplex cho proxy bơm traffic. **SOCKS5 UDP-ASSOCIATE** dùng [`VpnUdpAssociateSource` @ :22](../demo/Vpn2ProxyDemo/VpnProxySource.VpnUdpAssociateSource.cs#L22)
...
| [VpnCapabilityProbe.cs:23](../demo/Vpn2ProxyDemo/VpnCapabilityProbe.cs#L23) | **Static probe** (mirror `UdpDnsProbe`): [`RunAsync` @ :26](../demo/Vpn2ProxyDemo/VpnCapabilityProbe.cs#L26) gộp probe thật (UDP qua `UdpDnsProbe`; LAN ảo qua [`ProbeVirtualLanAsync` @ :106](../demo/Vpn2ProxyDemo/VpnCapabilityProbe.cs#L106) → `PingAsync`, trả kèm gateway phát hiện) + năng lực driver + heuristic NAT/IPv6 → `VpnCapabilityReport`; mỗi sub-probe tự timeout, không ném khi hết giờ |
...
| LAN ảo trong VPN | ~ (probe) | ICMP ping gateway nội bộ + DNS cùng /24 (phát hiện ⇒ panel thêm dòng **Gateway nội bộ**) | SSTP/L2TP/OpenVPN-tun là model **điểm-điểm** (`MultiHostModel.None`); **SoftEther** và **OpenVPN-tap** đã khai `MultiHostModel.L2BroadcastDomain` (L2 Ethernet qua `EthernetAdapter`, L2.8). Demo vẫn chỉ probe ICMP gateway nội bộ (chưa duyệt host L2) — quét/định danh host trong broadcast domain → [`11` L2.4–L2.8](11-todo-roadmap.md) (NDISC/DHCP/`EthernetAdapter`). |
| MAC address (L2) | ✗/? | tĩnh (`LinkLayer` của driver đang chạy) | SSTP/L2TP/OpenVPN-tun là `LinkLayer.L3Ip` (PPP/IPCP/config-push point-to-point) ⇒ không có khung Ethernet/MAC ⇒ panel báo `[✗] No`. **SoftEther** + **OpenVPN-tap** khai `LinkLayer.L2Ethernet` ⇒ panel báo `[?] Unknown` (có khung Ethernet nhưng demo chưa đọc `LinkAddress` từ kênh L2). Nền L2 đã có (`MacAddress`/`EthernetSwitch`/`VirtualHost`/`IEthernetChannel.LinkAddress`/`EthernetAdapter`) → còn lại: demo đọc & hiển thị MAC từ kênh L2 ([`11` L2.x](11-todo-roadmap.md)). |
