...
| **Drivers.L2tpIpsec** | [L2tpIpsecDriver.cs:9](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecDriver.cs#L9), [L2tpIpsecConnection.cs:25](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L25), [IpsecL2tpTransport.cs:11](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/IpsecL2tpTransport.cs#L11), [UdpEncapsulation.cs:8](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/UdpEncapsulation.cs#L8), [L2tpPppFrameChannel.cs:7](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpPppFrameChannel.cs#L7) | Driver điều phối **L2TP/IPsec**: lắp IKEv1 + ESP + L2TP + PPP thành 1 tunnel sau facade ổn định; vòng đời đầy đủ keepalive/rekey/teardown/auto-reconnect (§6/§6.1). **Refs**: Abstractions + Ppp + Transport.Udp + Ipsec + L2tp. Files **phẳng ở gốc project** (đã bỏ subfolder `L2tpIpsec/`); giữ `Enums/`+`Models/`. |
...
5. **Dựng ESP**: `BuildEspSession` lấy khóa Phase 2 → `EspSession` AES-CBC + HMAC-SHA1 (2 chiều); bật `_espActive` — [L2tpIpsecConnection.cs:149-151](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L149-L151) (factory [`BuildEspSession` @ :198](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L198)).
...
Sau `LinkUp`, [`StartKeepalive` @ :253](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L253) bật 4 timer; IKE inbound hậu-handshake đi vào [`HandleInboundIke` @ :301](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L301).
...
- **Rekey Phase 2** (make-before-break): ~90% lifetime (3600s) → [`RekeyPhase2Async` @ :334](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L334) chạy Quick Mode mới trên IKE-SA hiện có → `EspSession` mới; [`IpsecL2tpTransport.SwapSession` @ :32](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/IpsecL2tpTransport.cs#L32) chuyển outbound ngay, giữ SA cũ cho inbound thêm 10s ([`ScheduleDropPreviousInbound` @ :366](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L366)).
- **Auto-reconnect**: mọi tín hiệu rớt (DPD chết / server Delete / L2TP teardown / **Phase 1 hết hạn 8h** = rekey-by-reconnect) → [`OnLinkLost` @ :377](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L377) (lock chống double-supervisor + drop-window race) → [`ReconnectLoopAsync` @ :405](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L405) retry **backoff 1s→30s ×2 ±20%** (bật mặc định, [L2tpIpsecReconnectOptions.cs](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecReconnectOptions.cs)). Tunnel mới chui sau `_facade` ⇒ IP stack + flow **same-IP sống sót không re-bind**; đổi IP ⇒ cập nhật `Config` + [`L2tpIpsecVpnSession.Reconfigured` @ :29](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecVpnSession.cs#L29).
- **Teardown sạch**: [`DisconnectAsync` @ :492](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L492) (`IAsyncDisposable`) hủy reconnect đang chờ, gửi **CDN + StopCCN** (L2TP) + **Delete ESP/ISAKMP** (IKE) rồi mới đóng socket (best-effort, timeout 2s).
- **Trạng thái**: event [`StateChanged` @ :92](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L92) + enum [L2tpIpsecConnectionState.cs](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/Enums/L2tpIpsecConnectionState.cs) (Connecting/Connected/Reconnecting/Disconnected); event [`Reconnected` @ :95](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L95).
...
- ✅ **Phân loại lỗi typed** (`VpnConnectionException` + 3 lớp con auth/server-reject/network-timeout) ở cả 2 driver; caller `OperationCanceledException` không bị tái phân loại. Timeout vẫn hard-code (IKE 5×2.5s, L2TP retransmit 1s không giới hạn) — xem [roadmap §11](11-todo-roadmap.md).
...
- 11 project test trong `tests/`: **119 test offline pass** (`net8.0`) phủ từng tầng độc lập + bổ sung regression offline: **IKEv1 in-process two-party capstone** (Main Mode MM1-6 PSK + Quick Mode QM1-3 với responder giả lập, rồi ESP 2 chiều + DPD/Delete — `IkeV1HandshakeTests.cs`), **userspace TCP loopback** (`TcpStackTests.cs`), **fuzz/malformed parser** (`ParserFuzzTests.cs` ISAKMP V1 + IkeMessage V2, `L2tpCodecFuzzTests.cs`, `HdlcFuzzTests.cs`, `SstpControlCodecTests.cs`). Test live SSTP/L2TP (~12) vẫn gắn `[Trait("Category","Integration")]` (chạy offline bằng `--filter "Category!=Integration"`).
