...
| `OpenConnectTunnelInfo` | config tunnel: `Address`/`AddressV6`/`Netmask`/`PrefixLengthV6`/`DnsServers`/`Routes`/`Mtu`/`Dpd`/`Keepalive`/`RekeyMethod`/`RekeyTime`/`ParsedRekeyMethod` (V5.d)/`SessionCookie`; **DTLS** `DtlsSessionId`/`DtlsCipherSuite`/`DtlsPort`/`DtlsKeepalive`/`DtlsDpd`/`HasDtls` (V5.c); `ToTunnelConfig()` | [Models/OpenConnectTunnelInfo.cs:17](Models/OpenConnectTunnelInfo.cs#L17) |
