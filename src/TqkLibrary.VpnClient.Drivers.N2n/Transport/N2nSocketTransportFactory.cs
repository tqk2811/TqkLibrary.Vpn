...
{
    /// <summary>
    /// The production <see cref="IN2nTransportFactory"/>: opens a real connected UDP socket to the n2n supernode. n2n is
    /// UDP-only — one datagram is one message, no framing — so the transport is a thin <see cref="IDatagramTransport"/>
    /// plus a receive loop that raises each datagram to the connection's handler. The socket I/O is exercised live (lab);
    /// the offline tests drive the connection through an in-process factory. Mirrors <c>NebulaSocketTransportFactory</c>.
    /// </summary>
    public sealed class N2nSocketTransportFactory : IN2nTransportFactory
    {
        ...
        public N2nSocketTransportFactory(int receiveBufferSize = 65535)
        {
        ...
        /// <inheritdoc/>
        public async Task<N2nTransportHandle> ConnectAsync(IPEndPoint remote, CancellationToken cancellationToken)
        {
            ...
            return new N2nTransportHandle(socket, socket.SetReceiver, socket.RunReceiveLoopAsync);
        }
