...
- 11 project `src/` + 11 project `tests/`, build xanh `netstandard2.0`+`net8.0`, **119 test offline pass** (`net8.0`) — gồm IKEv1 capstone, TCP loopback, fuzz parser; thêm ~12 live integration `[Trait("Category","Integration")]` (chạy offline bằng `--filter "Category!=Integration"`).
...
- [ ] **Phân loại lỗi & timeout rõ ràng** — phần **typed exception đã xong**, phần **timeout cấu hình + retransmit cap còn lại**:
  ...
  - [ ] **Timeout cấu hình được** — IKE retransmit 5×2.5s hard-code, **L2TP control-channel retransmit 1s không giới hạn số lần** ([L2tpControlChannel.cs](../src/TqkLibrary.Vpn.L2tp/L2tpControlChannel.cs)). Chưa làm.
