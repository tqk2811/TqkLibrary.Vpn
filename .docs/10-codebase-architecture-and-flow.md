...
| **Ipsec.Ike.V1** | [IkeV1Client.cs:16](../src/TqkLibrary.Vpn.Ipsec/Ike/V1/IkeV1Client.cs#L16) | Bắt tay ISAKMP/IKEv1 cho L2TP: Main Mode (MM1–MM6) + Quick Mode (QM1–QM3) → sinh khóa ESP. **Đây là IKE đang chạy thực tế.** Thêm qua **Informational exchange**: DPD keepalive ([IkeV1Dpd.cs](../src/TqkLibrary.Vpn.Ipsec/Ike/V1/IkeV1Dpd.cs)), Quick Mode rekey (`BuildRekeyQuickMode1/2/3`), Delete teardown ([IkeV1Delete.cs](../src/TqkLibrary.Vpn.Ipsec/Ike/V1/IkeV1Delete.cs)). |
...
| **L2tp** | [L2tpClient.cs:12](../src/TqkLibrary.Vpn.L2tp/L2tpClient.cs#L12), [L2tpControlChannel.cs:10](../src/TqkLibrary.Vpn.L2tp/L2tpControlChannel.cs#L10) | L2TPv2 (RFC 2661): tunnel SCCRQ→SCCRP→SCCCN, session ICRQ→ICRP→ICCN; truyền tin cậy Ns/Nr + ZLB ack + retransmit 1s. PPP frame chui trong L2TP data. Keepalive `SendHelloAsync` (HELLO, §5.5); teardown `SendCallDisconnectAsync` (CDN) + `SendStopControlConnectionAsync` (StopCCN). |
...
| **Abstractions.Channels** | [SwappablePacketChannel.cs:18](../src/TqkLibrary.Vpn.Abstractions/Channels/SwappablePacketChannel.cs#L18) | Facade `IPacketChannel` **ổn định**: hot-swap kênh PPP bên trong khi reconnect, ghim metadata link, drop write khi chưa có inner. Để IP stack **không phải re-bind** qua reconnect (xem §6.1). |
...
8. **Cắm vào facade ổn định**: kênh PPP mới được nạp vào `_facade.SetInner(...)` ([L2tpIpsecConnection.cs:171](../src/TqkLibrary.Vpn.Drivers/L2tpIpsec/L2tpIpsecConnection.cs#L171)) rồi `StartKeepalive()`. Driver bọc thành [L2tpIpsecVpnSession.cs:11](../src/TqkLibrary.Vpn.Drivers/L2tpIpsec/L2tpIpsecVpnSession.cs#L11) trả về với `PacketChannel` = [`_facade`](../src/TqkLibrary.Vpn.Drivers/L2tpIpsec/L2tpIpsecConnection.cs#L39) (**bền qua reconnect**), IP/DNS từ IPCP.
