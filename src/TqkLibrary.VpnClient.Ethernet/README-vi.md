...
- **Quy tắc vàng** (design [00 §5](../../.docs/00-architecture-overview.md)): stack TCP/IP **chỉ** bind `IPacketChannel`, không bao giờ thấy Ethernet; mọi MAC/ARP/DHCP nằm ở tầng này. Vì vậy 2 *slot* `INeighborResolver` (ARP/NDISC) + `IAddressConfigurator` (DHCP/SLAAC) được khai báo trong `TqkLibrary.VpnClient.Abstractions` (dùng raw bytes/`IPAddress`, **không** ref `MacAddress`); `INeighborResolver` nay **đã có hiện thực IPv4 = `ArpResolver` (L2.3)**, NDISC v6 (L2.4) + `IAddressConfigurator` (DHCP/SLAAC, L2.5/L2.6) còn lại.
...
|------|---------|-------------------|--------|
| `INeighborResolver` | Resolve next-hop IP → MAC (raw 6 byte) | **ARP (L2.3) ✓ → [`ArpResolver`](ArpResolver.cs#L26)** / NDISC (L2.4) | [INeighborResolver.cs:12](../TqkLibrary.VpnClient.Abstractions/Channels/Interfaces/INeighborResolver.cs#L12) |
...
- **Chưa làm (phase sau):** L2.4 NDISC · L2.5 DHCPv4 · L2.6 SLAAC/DHCPv6 · L2.7 `EthernetAdapter` ráp · L2.8 capabilities/multi-host session · L2.9 test+docs tổng · **driver L2 đa-host** (SoftEther / OpenVPN-tap multi-host → uplink VPN cấp cả broadcast domain). *(OpenVPN tap **1-host** đã tiêu thụ fabric ở V2.h.)* Xem roadmap [11-todo-roadmap.md §P3](../../.docs/11-todo-roadmap.md).
