...
namespace TqkLibrary.VpnClient.SoftEther.DataChannel
{
    ...
    /// The parsing rules (and the over-size guards) mirror <see cref="SoftEtherDataFrameCodec.DecodeBlock"/>; this type
    /// only adds the incremental "read exactly N bytes" loop the stream needs. A clean EOF before any byte of a new block
    /// surfaces as an empty result so the caller can treat it as the peer closing the session.
    /// </summary>
    public sealed class SoftEtherDataBlockReader
    {
        ...
        /// <summary>
        /// Reads the next block and returns its frames, or an empty list if the peer closed the stream cleanly at a block
        /// boundary. Throws <see cref="FormatException"/> on a malformed block and <see cref="SoftEtherProtocolException"/>
        /// if the stream ends mid-block.
        /// </summary>
