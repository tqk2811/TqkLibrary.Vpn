...
| Được dùng bởi | [Drivers.L2tpIpsec](../TqkLibrary.VpnClient.Drivers.L2tpIpsec) | `L2tpIpsecConnection : ReconnectingVpnConnection<L2tpIpsecConnectionState>` (consumer F.6 #7, **driver cuối** — F.6 xong cho mọi driver; `L2tpIpsecReconnectOptions : VpnReconnectOptions` không thêm knob — rekey Phase1/Phase2 in-place + multi-session + DPD/HELLO giữ ngoài supervisor) |

> **Mọi protocol driver** nay đã migrate sang base (F.6 hoàn tất).
...
- **Đã migrate (F.6 — driver cuối)**: **L2TP/IPsec** (`L2tpIpsecConnection : ReconnectingVpnConnection<L2tpIpsecConnectionState>`) — phức tạp nhất: rekey **Phase 2** (ESP CHILD SA) + **Phase 1** (ISAKMP SA) in-place (cả hai make-before-break, swap SA) + **multi-session** L2TP best-effort + keepalive L2TP HELLO + IKE DPD, tất cả **giữ ngoài supervisor** trên 4 timer riêng; `StopAttemptLoop` dispose 4 timer + tắt `_espActive`; `CleanupAttemptResourcesAsync` còn xóa `_extraSessions` (session phụ không sống qua reconnect); `DisconnectAsync` gửi CDN/StopCCN + IKE Delete trước teardown base. Jitter retransmit IKE giữ helper riêng (`L2tpIpsecTimeoutOptions.RetransmitJitterFraction`) — khác jitter reconnect của base. Không đổi hành vi (test `Drivers.L2tpIpsec.Tests` + `L2tp.Tests` + `Ipsec.Ike.Tests` + `Drivers.Core.Tests` xanh). **F.6 nay xong cho mọi driver.**
