...
- **Đảo ngược phụ thuộc (Dependency Inversion):** các driver protocol (L2TP/IPsec, SSTP...), tầng transport và userspace IP stack đều lập trình theo các interface ở đây, không lệ thuộc trực tiếp lẫn nhau. Façade (`TqkLibrary.Vpn`) khám phá driver qua [`IVpnProtocolDriver`](Drivers/Interfaces/IVpnProtocolDriver.cs#L9) mà không cần biết hiện thực cụ thể.
...
| `INeighborResolver` | **Slot L2** (design 00 §5): resolve next-hop IP → MAC (raw 6 byte); chưa hiện thực — ARP (L2.3) / NDISC (L2.4) | [INeighborResolver.cs:12](Channels/Interfaces/INeighborResolver.cs#L12) |
| `IAddressConfigurator` | **Slot L2** (design 00 §5): cấp IP/DNS/route (trả `TunnelConfig`); chưa hiện thực — DHCPv4 (L2.5) / SLAAC+DHCPv6 (L2.6) | [IAddressConfigurator.cs:14](Channels/Interfaces/IAddressConfigurator.cs#L14) |
...
- `IByteStreamTransport`: `ConnectAsync` + `ReadAsync`/`WriteAsync` thuần (Memory). Hiện thực `TlsByteStream` (driver SSTP) bọc `TcpClient`+`SslStream`, honor `CancellationToken` cả 2 TFM.
...
- **Khung mở rộng tương lai (chưa wire bởi driver nào):** nhiều thành viên enum năng lực mới chỉ là khung cho protocol chưa hiện thực — `LinkMedium.Ethernet`, `VpnSecurityKind.Noise`/`.Mppe`, `VpnTransportKind.Dtls`/`.RawIp`, `MultiHostModel.L2BroadcastDomain`, `AddressAssignment.Dhcp`, `VpnAuthMethod.Saml`/`.Otp`. Riêng `IEthernetChannel` (L2) **đã có impl** [`EthernetSwitch.Port`](../TqkLibrary.Vpn.Ethernet/EthernetSwitch.Port.cs#L14) trên nền L2.0–L2.3 (switch học MAC + `VirtualHost` bridge L2↔L3 + ARP IPv4) — chỉ **chưa có driver L2 production** ráp end-to-end (cần NDISC/DHCP/`EthernetAdapter`). Các driver hiện có (L2TP/IPsec đang chạy, SSTP) chỉ dùng tập con (L3/IP, ESP/TLS, IPCP, PSK/UserPassword).
- **Slot L2 (khai báo phase L2.0):** `INeighborResolver` (ARP/NDISC) + `IAddressConfigurator` (DHCP/SLAAC) là 2 slot của `EthernetAdapter` tương lai (design 00 §5). Dùng raw bytes/`IPAddress` (không ref `MacAddress` của project `TqkLibrary.Vpn.Ethernet`) để giữ CORE không phụ thuộc codec L2. `INeighborResolver` **đã có impl IPv4** ([`ArpResolver`](../TqkLibrary.Vpn.Ethernet/ArpResolver.cs#L26), ARP/L2.3); còn NDISC IPv4 (L2.4) + `IAddressConfigurator` (DHCP/SLAAC, L2.5→L2.6) chưa.
