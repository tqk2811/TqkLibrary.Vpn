...
├─ ITlsKeyingMaterialExporter.cs   Scaffold tls-ekm (RFC 5705) — chưa hiện thực; net8 SslStream chưa có exporter — F.5a
...
| `OpenVpnControlChannel` | control channel client: ctor nhận `controlWrap?` (tls-auth/tls-crypt); `ConnectAsync(...)` reset + `SslStream` AuthenticateAsClient; `NegotiateKeyMaterialAsync(...)` key-method-2 → `OpenVpnKeyMaterial` (V2.d) + lộ `ServerKeyMethodOptions` (NCP-less mining IV_CIPHERS); `RequestConfigAsync(...)` PUSH_REQUEST→PUSH_REPLY (V2.e); `SendExitNotifyAsync(...)` explicit-exit-notify best-effort; `LocalSessionId`/`RemoteSessionId`/`KeyId`/`TlsStream` | [OpenVpnControlChannel.cs:22](OpenVpnControlChannel.cs#L22) |
| `OpenVpnTlsBridgeStream` *(internal)* | `Stream` in-memory cho SslStream: `WriteAsync`→fragment+gửi reliable (callback), `ReadAsync`←payload in-order qua `EnqueueInbound`; `CompleteInbound` đóng | [OpenVpnTlsBridgeStream.cs:12](OpenVpnTlsBridgeStream.cs#L12) |
...
| `ITlsKeyingMaterialExporter` | **scaffold F.5a (chưa hiện thực)**: `Export(label, context, length)` cho `tls-ekm` (RFC 5705); net8/ns2.0 `SslStream` chưa có exporter (chỉ net9+ / BouncyCastle TLS) | [ITlsKeyingMaterialExporter.cs:24](ITlsKeyingMaterialExporter.cs#L24) |
...
| `OpenVpnKeyMaterial` | key2 256B cipher-independent; `DeriveDataKeys(cipher, isServer?)` (AEAD) / `DeriveCbcDataKeys(cbcCipher, hmacKeyLen, isServer?)` (CBC) slice sau khi biết cipher (NCP) | [DataChannel/OpenVpnKeyMaterial.cs:8](DataChannel/OpenVpnKeyMaterial.cs#L8) |
...
- **`tls-ekm` (RFC 5705 — scaffold F.5a)**: seam khai báo nhưng **chưa hiện thực** — interface [`ITlsKeyingMaterialExporter`](ITlsKeyingMaterialExporter.cs) (`tls-ekm` suy data-channel keys thẳng từ TLS exporter, label `EXPORTER-OpenVPN-datakeys`, thay cho PRF key-method-2). Đã **kiểm chứng bằng reflection**: `SslStream` net8.0 (và netstandard2.0) **không** có `ExportKeyingMaterial`/`Export*`/`Keying` — `SslStream.ExportKeyingMaterial` chỉ có từ **.NET 9**. Hiện thực thật cần hoặc target net9+ hoặc đẩy control-channel TLS qua **BouncyCastle TLS** (`TlsContext.ExportKeyingMaterial`); vì cần handshake TLS thật dưới test (ngoài phạm vi F.5a offline) nên chỉ scaffold contract, client giữ key-method-2 (PRF). IV_PROTO bit [`OpenVpnPeerInfo.IvProtoTlsKeyExport`](DataChannel/OpenVpnPeerInfo.cs#L25) **chưa** set.
