...
- **17 VPN drivers** registered via `VpnClientBuilder.Use*`, almost all already **validated live** (see the [Status](#status) table):
  ...
  - **Built + offline-tested (live still pending):** **PPTP** (RFC 2637 GRE + MPPE + MS-CHAPv2 — pending raw-IP proto-47 + elevation; legacy/interop).
...
```
 ...
 └────────────┬──────────────┘     │ VpnNetworkStream             │
              │ IVpnProtocolDriver  └─────────────┬───────────────┘
 ┌────────────▼───────────────┐    ┌──────────────▼──────────────┐
 │ DRIVERS (17 × Use*)        │    │ TqkLibrary.VpnClient.IpStack │
