...
- [ ] **P1.1 — IPv6 trong tunnel (IPV6CP, RFC 5072)** — bật IPv6 end-to-end qua SSTP/L2TP. IP stack **đã dual-stack**. **Đã xong (1) IPV6CP negotiator link-local** ([`Ipv6cpNegotiator`](../src/TqkLibrary.VpnClient.Ppp/Ipv6cpNegotiator.cs#L14) thương lượng Interface-Identifier → `fe80::/64`) + wire opt-in vào [`PppEngine(enableIpv6)`](../src/TqkLibrary.VpnClient.Ppp/PppEngine.cs#L33) (event `Ipv6Up`/`AssignedAddressV6`, song song IPCP, không đổi link-up IPv4) + test offline (as-built [`10`](10-codebase-architecture-and-flow.md) §5/§8/§9). **Còn lại**: (2) địa chỉ **global** qua SLAAC-từ-RA trên link hoặc DHCPv6 (tùy server — tái dùng codec NDISC khi có **L2.4**); (3) đẩy `AssignedAddressV6` lên [`TunnelConfig`](../src/TqkLibrary.VpnClient.Abstractions/Drivers/Models/TunnelConfig.cs#L12) + bật `enableIpv6` ở 2 driver, dựng stack qua overload dual-stack [`TcpIpStack(channel, v4, v6)` @ :49](../src/TqkLibrary.VpnClient.IpStack/TcpIpStack.cs#L49) ở **cả 2 driver**; (4) demo bật `IsSupportIpv6=true` + bỏ chặn đích IPv6 ở `VpnConnectSource`/`VpnUdpAssociateSource`. **Rủi ro**: đa số server VPN Gate chỉ cấp IPv4 ⇒ test live cần lab (**Q.1**, SoftEther/RRAS bật IPv6 pool).
...
- [ ] **F.7 — IKEv2 hoàn thiện protocol-level (còn lại)**. **Đã xong (code + test offline)**: (1) **Configuration Payload (CP)** — [`ConfigurationPayload`](../src/TqkLibrary.VpnClient.Ipsec/Ike/V2/Payloads/ConfigurationPayload.cs#L16) CFG_REQUEST/REPLY (virtual IP/netmask/DNS), opt-in `IkeClient(requestConfiguration)`; (2) **INFORMATIONAL** — DPD liveness + DELETE ([`DeletePayload`](../src/TqkLibrary.VpnClient.Ipsec/Ike/V2/Payloads/DeletePayload.cs#L13)) teardown CHILD_SA/IKE SA + `BuildInformationalResponse`; (3) **CREATE_CHILD_SA** — rekey **CHILD SA** make-before-break (`BuildRekeyChildSaRequest`/`ProcessRekeyChildSaResponse` → [`ChildSaParameters`](../src/TqkLibrary.VpnClient.Ipsec/Ike/V2/Models/ChildSaParameters.cs#L11), mirror IKEv1 Phase 2). **Còn lại**: (3b) **rekey IKE SA** qua CREATE_CHILD_SA (Phase-1 equivalent: SPI mới + KE/DH + dẫn xuất lại SK_*, mirror rekey Phase 1 in-place của IKEv1); (4) **EAP-MSCHAPv2** (tái dùng MS-CHAPv2 từ Ppp) cho server username/password — PSK trước, EAP sau cùng. Consumer: V.1.
...
- [ ] **V.1 — IKEv2-native** (tương thích strongSwan/libreswan; **rẻ nhất** — protocol đã build): driver `Drivers.Ikev2` mới — IKE_SA_INIT + IKE_AUTH (PSK trước) → **CP** nhận virtual IP/DNS (F.7) → **ESP tunnel mode** ([`EspTunnelChannel`](../src/TqkLibrary.VpnClient.Ipsec/Esp/EspTunnelChannel.cs#L14), F.8 ✓) → `IPacketChannel` trực tiếp (**không PPP**); NAT-T detect + float 4500 (tái dùng [`NatDetection`](../src/TqkLibrary.VpnClient.Ipsec/Ike/V2/NatDetection.cs) + `Ipsec/Nat`); keepalive DPD + rekey CHILD_SA (F.7) + teardown DELETE; supervisor/reconnect tái dùng F.6. Sau cùng: EAP-MSCHAPv2 cho server username/password. Lab: strongSwan Docker (Q.1).
- [ ] **V.2 — OpenVPN** (tương thích OpenVPN community server, làm theo phase):
  ...
  - [ ] **V2.d** key derivation (key method 2 TLS-PRF / `tls-ekm` — F.5) + **data channel** AES-256-GCM (tái dùng [`AesGcmCipher`](../src/TqkLibrary.VpnClient.Crypto/Aead/AesGcmCipher.cs)) + packet-id anti-replay (mirror mô hình anti-replay ESP) — consumer thật cho `ISecuritySession` F.2;
  - [ ] **V2.e** PUSH_REQUEST/PUSH_REPLY (ifconfig/route/DNS → [`TunnelConfig`](../src/TqkLibrary.VpnClient.Abstractions/Drivers/Models/TunnelConfig.cs)); topology `subnet`/`net30`; keepalive ping/ping-restart; **soft-reset renegotiation** (TLS rekey make-before-break — mirror swap `EspSession`);
...
2. **V.1 IKEv2-native** + F.7 (F.8 ESP tunnel mode đã xong — `EspTunnelChannel`) (driver mới đầu tiên, chi phí thấp nhất — validate mô hình F + F.6 supervisor chung).
