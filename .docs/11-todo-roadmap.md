...
- [ ] **P0.8 — Fallback khi gateway từ chối forced-NAT-T** (rủi ro #1 plan gốc): **(a) phát hiện + phân loại đã xong** (as-built ở [`10`](10-codebase-architecture-and-flow.md) §6 bước 4/§8/§9): [`IkeV1Client.TryReadRejectNotify`](../src/TqkLibrary.Vpn.Ipsec/Ike/V1/IkeV1Client.cs#L18) + [`ExchangeIkeAsync`](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L221) — NOTIFY lỗi (NO-PROPOSAL-CHOSEN…) ⇒ `VpnServerRejectedException`, im lặng sau float UDP/4500 ⇒ `VpnNetworkTimeoutException` chỉ rõ nghi forced-NAT-T (test offline [`IkeV1RejectNotifyTests`](../tests/TqkLibrary.Vpn.Ipsec.Ike.Tests/IkeV1RejectNotifyTests.cs)). **Còn lại** (cần **lab Docker Q.1** kiểm chứng từng server): (b) **retry bind src-port 500/4500 thật** khi cổng trống (vẫn UDP userspace, không cần raw; Windows thử-bind để tránh đụng IKEEXT, fail thì bỏ qua); (c) **native ESP proto-50** qua `Transport.RawIp` (elevate) — phụ thuộc **F.9**, làm sau cùng.
...
- [ ] **P0.10 — Sửa mâu thuẫn nội bộ [`10`](10-codebase-architecture-and-flow.md) §3**: bảng hợp đồng ghi `IEthernetChannel` "**chưa có implementation**" trong khi §5/§9 ghi [`EthernetSwitch.Port`](../src/TqkLibrary.Vpn.Ethernet/EthernetSwitch.Port.cs#L14) đã implement (L2.1) — sửa dòng §3 cho khớp (đã có impl src; driver L2 production thì chưa).
- [ ] **P0.11 — Cấu trúc thư mục `IpStack/Tcp/`**: [UdpConnection.cs](../src/TqkLibrary.Vpn.IpStack/Tcp/UdpConnection.cs) và [TcpIpStack.cs](../src/TqkLibrary.Vpn.IpStack/Tcp/TcpIpStack.cs) (demux cả UDP/ICMP/ping) nằm trong `Tcp/`, namespace `TqkLibrary.Vpn.IpStack.Tcp` — chuyển `TcpIpStack` ra gốc project, `UdpConnection` (+`UdpReceiveResult`) sang `Udp/`, namespace đổi theo thư mục (**breaking nội bộ**) ⇒ cập nhật `using` ở `Sockets`/demo/tests + toàn bộ link trong README/`10`/`11`.
...
- [ ] **P1.4 — Exponential backoff cho retransmit IKE + L2TP control**: khoảng retransmit hiện cố định mỗi tick (`IkeRetransmitInterval` 2.5s / L2TP 1s) — nhân đôi dần có jitter, giữ cap số lần qua `L2tpIpsecTimeoutOptions` sẵn có.
- [ ] **P1.5 — Timeout chủ động cho `SstpTransport.ReadPacketAsync`**: hiện chỉ dừng khi TLS stream đóng/cancel — thêm read-timeout cấu hình (phát hiện server treo giữa chừng handshake/data).
- [ ] **P1.6 — Test offline supervisor/keepalive/reconnect SSTP**: chèn fake stream qua seam `ITlsByteStream` + `Func<ITlsByteStream>` factory (**đã có ở P0.1** — [SstpConnection ctor :69](../src/TqkLibrary.Vpn.Drivers.Sstp/SstpConnection.cs#L69), framing đã test offline ở [SstpTransportSeamTests](../tests/TqkLibrary.Vpn.Sstp.Tests/SstpTransportSeamTests.cs)); còn lại: fake cấp **cert giả** cho crypto-binding để chạy trọn handshake→keepalive→reconnect không cần server. Hiện supervisor SSTP mới phủ qua live integration test. → [SstpConnection.cs](../src/TqkLibrary.Vpn.Drivers.Sstp/SstpConnection.cs).
...
- [ ] **F.9 — `Transport.RawIp`** (opt-in, cần elevate, tự detect quyền Administrators/CAP_NET_RAW — **ngoại lệ có chủ đích** của ràng buộc no-admin design `01`): raw socket IP protocol tùy ý. Consumer: PPTP/GRE (V.6), native ESP proto-50 (P0.8c), EtherIP/L2TPv3 (tương lai).
...
- [ ] **Q.5 — CI đa OS** (plan M0) — chưa có cấu hình CI; chạy offline suite 2 TFM + (sau Q.1) integration lab.
