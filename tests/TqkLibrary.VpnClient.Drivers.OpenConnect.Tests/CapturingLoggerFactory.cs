...
namespace TqkLibrary.VpnClient.Drivers.OpenConnect.Tests
{
    ...
    /// driver emits (auth/connect/DPD/reconnect/drop). It records every entry's <see cref="EventId"/> and formatted
    /// message so a test can assert that an expected event fired — without touching the console or any real sink.
    /// </summary>
