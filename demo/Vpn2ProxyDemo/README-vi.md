...
CLI gồm 3 subcommand; target VPN gói trong `--vpn` (URI). Option:
...
- Demo tham chiếu project tới [`src/TqkLibrary.VpnClient.Drivers`](../../src/TqkLibrary.VpnClient.Drivers) (driver façades) và [`src/TqkLibrary.VpnClient.Sockets`](../../src/TqkLibrary.VpnClient.Sockets) (`VpnTcpClient`/`VpnUdpClient`/`TcpIpStack`), cùng NuGet `TqkLibrary.Proxy` 1.0.35 + `Microsoft.Extensions.Logging`/`.Console` 10.0.x.
