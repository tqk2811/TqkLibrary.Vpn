...
{
    /// <summary>The single PPP/L3 session of an SSTP connection. <see cref="PacketChannel"/> is the stable facade.</summary>
    public sealed class SstpVpnSession : IVpnSession
    {
        ...
        /// <summary>
        /// Raised after an auto-reconnect updates <see cref="Config"/>. When <see cref="SstpReconnectInfo.AddressChanged"/>
        /// is true the consumer should rebuild its IP stack; otherwise existing in-tunnel sockets keep working.
        /// </summary>
        public event Action<SstpReconnectInfo>? Reconfigured;

        /// <summary>Applies a reconnect's new address/DNS to <see cref="Config"/> and raises <see cref="Reconfigured"/>.</summary>
        internal void ApplyReconnect(SstpReconnectInfo info, IPAddress? dns)
        {
