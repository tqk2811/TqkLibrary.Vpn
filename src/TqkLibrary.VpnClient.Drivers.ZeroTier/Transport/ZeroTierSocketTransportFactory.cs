...
{
    /// <summary>
    /// The production <see cref="IZeroTierTransportFactory"/>: opens a real connected UDP socket to the ZeroTier node /
    /// controller. ZeroTier VL1 is UDP — one datagram is one packet, no framing — so the transport is a thin
    /// <see cref="IDatagramTransport"/> plus a receive loop that raises each datagram to the connection's handler. The
    /// socket I/O is exercised live (lab); the offline tests drive the connection through an in-process factory. Mirrors
    /// <c>N2nSocketTransportFactory</c>.
    /// </summary>
    public sealed class ZeroTierSocketTransportFactory : IZeroTierTransportFactory
    {
        ...
        public ZeroTierSocketTransportFactory(int receiveBufferSize = 65535)
        {
        ...
        /// <inheritdoc/>
        public async Task<ZeroTierTransportHandle> ConnectAsync(IPEndPoint remote, CancellationToken cancellationToken)
        {
            ...
            return new ZeroTierTransportHandle(socket, socket.SetReceiver, socket.RunReceiveLoopAsync);
        }

        /// <summary>A connected UDP datagram pipe over a real socket (live-only; mirrors the n2n / Nebula UDP socket).</summary>
