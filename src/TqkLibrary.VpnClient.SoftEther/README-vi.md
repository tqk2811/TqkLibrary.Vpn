...
> - **V.4 payload codec `use_compress`/`use_encrypt` xong** (code + test offline). `SoftEtherPayloadCompressor` nén **per-frame** bằng DEFLATE (`DeflateStream`, magic `0xDEADBEEFCAFEFACE`, chỉ nén khi nhỏ hơn) trước khi block-encode; `SoftEtherEncryptedTransport` (decorator `IByteStreamTransport`) áp RC4 keystream **per-direction** dưới block framing, **tái dùng** [`Rc4`](../TqkLibrary.VpnClient.Crypto/Rc4.cs) (F.5b), khóa suy từ session key + nhãn hướng (`CreateClient`/`CreateServer` mirror nhau). Thứ tự egress **compress-rồi-encrypt**. **Mặc định tắt** (payload thô trên TLS) — RC4 broken (RFC 7465) chỉ để interop legacy trên TLS.
...
│  ├─ SoftEtherEthernetChannel.cs  IEthernetChannel: WriteFrameAsync (encode block, nén nếu useCompress) + Deliver (raise InboundFrame, giải-nén nếu useCompress, drop keep-alive)
│  ├─ SoftEtherPayloadCompressor.cs    V.4 use_compress: DEFLATE per-frame (magic 0xDEADBEEFCAFEFACE, chỉ nén khi nhỏ hơn) — static codec thuần
│  ├─ SoftEtherEncryptedTransport.cs   V.4 use_encrypt: decorator IByteStreamTransport áp RC4 per-direction (tái dùng Crypto.Rc4) dưới block framing
...
| `PackValueType` | enum tag wire (uint): `Int=0`/`Data=1`/`Str=2`/`UniStr=3`/`Int64=4` | [Pack/Enums/PackValueType.cs:8](Pack/Enums/PackValueType.cs#L8) |
...
| `SoftEtherAnsi` | codec STR "ANSI" = Latin-1 8-bit (`GetBytes`/`GetString`), đồng nhất 2 TFM | [Pack/SoftEtherAnsi.cs:9](Pack/SoftEtherAnsi.cs#L9) |
...
| `SoftEtherWatermark` | builder POST watermark: `BuildRequest(host)` (POST line + headers + signature‖padding), `Matches(body)`, `WithSignature`/`WithRandomPadding` | [SoftEtherWatermark.cs:20](SoftEtherWatermark.cs#L20) |
| `SoftEtherAuth` | codec auth (nhận `IHashAlgo`=SHA-0): `HashPassword`/`SecureFromHashedPassword`/`ComputeSecurePassword` (password gốc **không** qua wire) | [SoftEtherAuth.cs:18](SoftEtherAuth.cs#L18) |
| `SoftEtherHttpPackCodec` | framing PACK trong thân HTTP: `FrameBody`/`ParseBody` (uint32 len + PACK) + `BuildPostRequest`/`BuildOkResponse` | [SoftEtherHttpPackCodec.cs:14](SoftEtherHttpPackCodec.cs#L14) |
| `SoftEtherHandshake` | state machine: `ParseHello`/`BuildLoginPack`/`ParseWelcome` (thuần) + `RunAsync(IByteStreamTransport)` + reader HTTP nội bộ; **V.4 multi-connection**: `BuildAdditionalConnectPack`/`ParseAdditionalConnectReply` (thuần) + `RunAdditionalConnectAsync` (watermark→hello→`additional_connect`+session key→ack) | [SoftEtherHandshake.cs:22](SoftEtherHandshake.cs#L22) |
| `SoftEtherProtocolException` | lỗi protocol (hello hỏng / login từ chối), mang `ErrorCode` từ field `error` của server | [SoftEtherProtocolException.cs:8](SoftEtherProtocolException.cs#L8) |
| `SoftEtherDataFrameCodec` | V4.c codec block thuần: `EncodeBlock`/`EncodeSingle` (`uint32(n)·{uint32(size)·bytes}`), `DecodeBlock`, `IsKeepAlive` | [DataChannel/SoftEtherDataFrameCodec.cs:13](DataChannel/SoftEtherDataFrameCodec.cs#L13) |
| `SoftEtherDataBlockReader` | V4.c reader streaming: `ReadBlockAsync` ráp 1 block qua partial read; EOF ranh block ⇒ list rỗng, EOF giữa block ⇒ `SoftEtherProtocolException` | [DataChannel/SoftEtherDataBlockReader.cs:15](DataChannel/SoftEtherDataBlockReader.cs#L15) |
| `SoftEtherEthernetChannel` | V4.c `IEthernetChannel`: `WriteFrameAsync` (encode block 1-frame → sink, nén DEFLATE nếu `useCompress`), `Deliver` (raise `InboundFrame`, giải-nén nếu `useCompress`, drop keep-alive); MAC + `MaxHeaderLength`=14 + `RequiresLinkAddressResolution` | [DataChannel/SoftEtherEthernetChannel.cs:19](DataChannel/SoftEtherEthernetChannel.cs#L19) |
| `SoftEtherPayloadCompressor` | V.4 `use_compress` static codec thuần: `CompressFrame` (DEFLATE per-frame + magic `0xDEADBEEFCAFEFACE`, chỉ nén khi nhỏ hơn) / `DecompressFrame` (inflate nếu có magic, hỏng/vượt `MaxFrameSize` ⇒ `FormatException`) / `IsCompressed` | [DataChannel/SoftEtherPayloadCompressor.cs:23](DataChannel/SoftEtherPayloadCompressor.cs#L23) |
| `SoftEtherEncryptedTransport` | V.4 `use_encrypt` decorator `IByteStreamTransport`: RC4 keystream **per-direction** dưới block framing (tái dùng [`Rc4`](../TqkLibrary.VpnClient.Crypto/Rc4.cs)); `CreateClient`/`CreateServer` (mirror), `DeriveDirectionKeys` (session key + nhãn hướng 1 byte) | [DataChannel/SoftEtherEncryptedTransport.cs:22](DataChannel/SoftEtherEncryptedTransport.cs#L22) |
| `SoftEtherMultiConnectionMux` | V.4 gộp 1-32 `IByteStreamTransport` = 1 data path: `SendBlockAsync` round-robin các connection gửi-được, `StartReceiveLoops` 1 decode loop/connection nhận-được → merge `InboundFrame`, `linkLost` callback **một lần** (one-shot), `DisposeAsync` hủy loop + đóng mọi socket | [DataChannel/SoftEtherMultiConnectionMux.cs:26](DataChannel/SoftEtherMultiConnectionMux.cs#L26) |
| `SoftEtherConnectionDirectionPlanner` | V.4 thuần: `Plan(count, halfConnection)` → mảng `SoftEtherConnectionDirection` (full-duplex ⇒ mọi `Both`; half ⇒ floor(n/2) `Send` + còn lại `Receive`, ≥1 mỗi hướng; 1 connection ⇒ `Both`) | [DataChannel/SoftEtherConnectionDirectionPlanner.cs:19](DataChannel/SoftEtherConnectionDirectionPlanner.cs#L19) |
| `SoftEtherConnectionDirection` | V.4 enum hướng 1 connection: `Both=0`/`Send=1`/`Receive=2` | [DataChannel/Enums/SoftEtherConnectionDirection.cs:9](DataChannel/Enums/SoftEtherConnectionDirection.cs#L9) |
...
| `SoftEtherAuthType` | enum authtype: `Anonymous=0`/`Password=1`/`PlainPassword=2`/`Certificate=3` | [Enums/SoftEtherAuthType.cs:8](Enums/SoftEtherAuthType.cs#L8) |
| `SoftEtherSessionParams` | record session: `MaxConnection`/`UseEncrypt`(mặc định `false`)/`UseCompress`(mặc định `false`)/`HalfConnection`/`UniqueId` | [Models/SoftEtherSessionParams.cs:10](Models/SoftEtherSessionParams.cs#L10) |
| `SoftEtherLoginRequest` | record input login: `HubName`/`UserName`/`AuthType`/`Password`/`Session` | [Models/SoftEtherLoginRequest.cs:11](Models/SoftEtherLoginRequest.cs#L11) |
| `SoftEtherHelloInfo` | record hello đã parse: `Hello`/`Version`/`Build`/`Random`(20B) | [Models/SoftEtherHelloInfo.cs:8](Models/SoftEtherHelloInfo.cs#L8) |
| `SoftEtherWelcomeInfo` | record welcome đã parse: `SessionKey`(DATA 20B `session_key`)/`SessionKey32`(INT `session_key_32`) + **V.4** `MaxConnection`(server cấp 1–32, default 1)/`HalfConnection` | [Models/SoftEtherWelcomeInfo.cs:9](Models/SoftEtherWelcomeInfo.cs#L9) |
...
1. **Watermark POST** ([`SoftEtherWatermark.BuildRequest`](SoftEtherWatermark.cs#L73)): ghi `POST /vpnsvc/connect.cgi HTTP/1.1` + headers (`Host`/`Content-Type: image/jpeg`/`Content-Length`/`Connection: Keep-Alive`) + thân = signature ‖ padding. **Signature** mặc định là blob **placeholder** byte-exact tái tạo được ([`DefaultSignature`](SoftEtherWatermark.cs#L27)) — **KHÔNG** phải blob GPL `Watermark.c`; nối server thật thì truyền blob thật qua [`WithSignature`](SoftEtherWatermark.cs#L52).
2. **Đọc hello** ([`ReadHttpPackAsync`](SoftEtherHandshake.cs#L237) → [`ParseHello`](SoftEtherHandshake.cs#L41)): đọc 1 message HTTP (header tới `\r\n\r\n` + `Content-Length`), parse thân qua [`SoftEtherHttpPackCodec.ParseBody`](SoftEtherHttpPackCodec.cs#L33) → PACK `hello`/`version`/`build`/`random`(20B challenge).
3. **Dựng & gửi login** ([`BuildLoginPack`](SoftEtherHandshake.cs#L68) → [`SoftEtherHttpPackCodec.BuildPostRequest`](SoftEtherHttpPackCodec.cs#L48)): PACK `method=login`/`hubname`/`username`/`authtype` + credential + session params. **Auth password**: [`SoftEtherAuth.ComputeSecurePassword`](SoftEtherAuth.cs#L76) = `SHA0(SHA0(password‖UPPER(username))‖server_random)` (tái dùng [`Sha0`](../TqkLibrary.VpnClient.Crypto/Sha0.cs) qua `IHashAlgo`) — password gốc **không** qua wire. POST tới `/vpnsvc/vpn.cgi`.
4. **Đọc welcome / lỗi** ([`ParseWelcome`](SoftEtherHandshake.cs#L110)): field `error` ≠ 0 ⇒ `SoftEtherProtocolException(ErrorCode)`; else trả DATA **`session_key`** (20B handle — KHÔNG phải STR `session_name`) + INT `session_key_32` + **`max_connection`** (server cấp 1–32, mặc định 1) + **`half_connection`**.
...
1. **Mở connection phụ** ([`RunAdditionalConnectAsync`](SoftEtherHandshake.cs#L209)): mỗi connection phụ chạy watermark POST → đọc hello (bỏ qua challenge — không auth lại) → POST PACK `method=additional_connect` + DATA **`session_key`**=session key (server `GetSessionFromKey` gắn socket vào session đã login) → đọc ack (`error`≠0 ⇒ từ chối) + leftover. Codec thuần: [`BuildAdditionalConnectPack`](SoftEtherHandshake.cs#L140)/[`ParseAdditionalConnectReply`](SoftEtherHandshake.cs#L155).
...
- **Payload codec (V.4 `use_compress`/`use_encrypt`) đã có**: `SoftEtherPayloadCompressor` nén DEFLATE per-frame (magic `0xDEADBEEFCAFEFACE`, chỉ nén khi có lợi) hook vào `SoftEtherEthernetChannel` qua cờ `useCompress`; `SoftEtherEncryptedTransport` bọc `IByteStreamTransport` áp RC4 per-direction (tái dùng `Crypto.Rc4`) dưới block framing, suy 2 keystream từ session key + nhãn hướng. Thứ tự egress **compress-rồi-encrypt** (decrypt-rồi-decompress khi nhận). **Mặc định tắt** (`SoftEtherSessionParams.UseEncrypt`/`UseCompress` = `false`) — payload thô trên TLS, không vỡ test cũ; RC4 broken (RFC 7465) chỉ để interop legacy trên TLS. Driver wire theo `login.Session.UseCompress`/`UseEncrypt` (RC4 bọc sau handshake plaintext, khóa `welcome.SessionKey`). Test offline `SoftEtherPayloadCodecTests` (compressor + RC4 mirror + block round-trip 4 tổ hợp) + driver `Connect_WithEncryptAndCompress_RoundTripsIp`.
- **VALIDATE LIVE (V.4) xong**: handshake + data plane interop với SoftEther server thật — bug interop đã sửa (HTTP-PACK framing bỏ length-prefix, field `session_key`/`session_key_32`, data trong SSL qua `use_encrypt`, keep-alive block magic `0xFFFFFFFF` qua `EncodeKeepAlive`, re-inject leftover qua `PrefixedByteStreamTransport`). Chi tiết + residual ở [`Drivers.SoftEther`](../TqkLibrary.VpnClient.Drivers.SoftEther/README-vi.md) + lab [`lab/softether`](../../lab/softether/README-vi.md). Watermark gỡ blocker bằng hằng `"VPNCONNECT"` (không cần blob GPL). Lộ trình V.4 ở [`.docs/11`](../../.docs/11-todo-roadmap.md) §V.4; design-intent SoftEther ở [`.docs/07`](../../.docs/07-softether.md).
