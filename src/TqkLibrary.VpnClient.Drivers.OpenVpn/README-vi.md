...
- **Teardown** (`DisconnectAsync` mặc định của base ⇒ `DisconnectCoreAsync`): hủy reconnect đang chờ, hủy receive loop, dispose control channel + socket (`CleanupAttemptResourcesAsync` của driver). (Explicit-exit-notify để best-effort sau.)
