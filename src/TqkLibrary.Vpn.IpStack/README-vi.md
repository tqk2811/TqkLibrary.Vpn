...
- `TcpIpStack.ConnectAsync(IPAddress, ushort, CancellationToken)` → [TcpIpStack.cs:40](Tcp/TcpIpStack.cs#L40): cấp local port ephemeral (bắt đầu 49152), tạo `TcpConnection`, gửi SYN và chờ `Connected`.
- `TcpIpStack.BindUdp()` / `BindUdp(ushort)` → [TcpIpStack.cs:56](Tcp/TcpIpStack.cs#L56): bind socket UDP userspace.
- `TcpIpStack.PingAsync(IPAddress, ReadOnlyMemory<byte>, CancellationToken)` → [TcpIpStack.cs:71](Tcp/TcpIpStack.cs#L71): cấp sequence, gửi ICMP Echo Request, chờ Echo Reply khớp identifier/sequence → `PingReply` (RTT); nhận Destination Unreachable ⇒ ném `IcmpUnreachableException`.
...
2. Mỗi gói IPv4 inbound vào `OnInbound` — [TcpIpStack.cs:111](Tcp/TcpIpStack.cs#L111): **trước tiên đưa qua `Ipv4Reassembler.Offer`** [TcpIpStack.cs:116](Tcp/TcpIpStack.cs#L116) — gói nguyên đi thẳng, fragment được buffer (return `null` ⇒ bỏ qua tới khi ráp xong); sau đó đọc protocol [Ipv4.cs:97](Ipv4.cs#L97); nếu TCP (6) lấy **destination port = local port của ta** [TcpIpStack.cs:126](Tcp/TcpIpStack.cs#L126) rồi tra `TcpConnection`; nếu UDP (17) tra `UdpConnection` theo local port [TcpIpStack.cs:134](Tcp/TcpIpStack.cs#L134); nếu ICMP (1) vào `OnIcmp` [TcpIpStack.cs:142](Tcp/TcpIpStack.cs#L142).
...
`OnIcmp` — [TcpIpStack.cs:146](Tcp/TcpIpStack.cs#L146) phân loại theo type:

- **Echo Request (8) → tự trả lời:** build Echo Reply echo lại payload + identifier/sequence rồi gửi về source — [TcpIpStack.cs:151-157](Tcp/TcpIpStack.cs#L151-L157). Host được cấp IP trong tunnel vì thế đáp ping của bên khác.
- **Echo Reply (0) → khớp ping đang chờ:** chỉ nhận nếu `identifier == _pingIdentifier` (per-stack, random), tra `_pings` theo sequence rồi complete `PingReply` — [TcpIpStack.cs:159-165](Tcp/TcpIpStack.cs#L159-L165).
- **Destination Unreachable (3) → fail ping:** trích datagram bị quote (IP header + 8 byte = Echo Request đã gửi), đọc identifier/sequence để định danh ping rồi ném `IcmpUnreachableException(code)` — [TcpIpStack.cs:166-179](Tcp/TcpIpStack.cs#L166-L179).
