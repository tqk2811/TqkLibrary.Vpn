...
- **OFFLINE**: 32 test [`Vtun.Tests`](../../tests/TqkLibrary.VpnClient.Vtun.Tests) — message 50-byte round-trip, challenge a..p encode/decode, **golden vector OpenSSL challenge** (`MD5("pass")` → `BF-ECB(0x00..0x0f)` = `7416f64c8c4581f8a271117a81d15366`, byte-for-byte), flags parse/encode (`<Tu>`/`<TuE1>`/`<TuC6K>`/`<TuS256>`), frame data/control header (ECHO_REQ 0x2000/ECHO_REP 0x4000/CONN_CLOSE/BAD_FRAME/oversized), **+9 test encryptor**: 3 golden vector OpenSSL legacy-provider BF-ECB byte-exact (`"pass"` 5B→`771cf8354baedcbf`, 3B→`93f42f60588e4ffa`; `"secret123"` "Hi"→`871286fb039e78f3`) + round-trip mọi length + block-aligned full-pad + factory resolve + key-derivation 32-byte. Build XANH ns2.0 + net8.
