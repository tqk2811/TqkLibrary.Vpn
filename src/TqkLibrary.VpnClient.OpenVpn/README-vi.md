...
│  ├─ OpenVpnDataCipher.cs         Catalog cipher NCP (AES-256/128-GCM): name/keySize/factory + resolve + IV_CIPHERS
│  ├─ OpenVpnPeerInfo.cs           Build IV_* peer-info (IV_PROTO/IV_NCP/IV_CIPHERS) cho key-method-2
...
| `OpenVpnDataCipher` | catalog cipher NCP: `Aes256Gcm`/`Aes128Gcm`, `TryResolve(name)`, `AdvertisedList`, `KeySizeBytes`, `CreateCipher()` | [DataChannel/OpenVpnDataCipher.cs:14](DataChannel/OpenVpnDataCipher.cs#L14) |
| `OpenVpnPeerInfo` | static `Build(ciphers?, ivProto?, …)` → khối `IV_*` (IV_PROTO/IV_NCP=2/IV_CIPHERS) cho key-method-2 | [DataChannel/OpenVpnPeerInfo.cs:13](DataChannel/OpenVpnPeerInfo.cs#L13) |
...
- **NCP** ([`OpenVpnDataCipher`](DataChannel/OpenVpnDataCipher.cs) + [`OpenVpnPeerInfo`](DataChannel/OpenVpnPeerInfo.cs)): client chào danh sách cipher qua peer-info `IV_CIPHERS=AES-256-GCM:AES-128-GCM` (+ `IV_PROTO`/`IV_NCP=2`) trong key-method-2; server chọn 1 và trả ở `PUSH_REPLY` (`cipher …`); `OpenVpnDataCipher.TryResolve(name)` map lại (key size + factory AEAD). Vì cipher chỉ biết **sau PUSH_REPLY**, suy khóa tách 2 bước: [`OpenVpnKeyMethod2.DeriveKey2`](DataChannel/OpenVpnKeyMethod2.cs) (key2 256B, **cipher-independent**, do `NegotiateAsync` trả qua [`OpenVpnKeyMaterial`](DataChannel/OpenVpnKeyMaterial.cs)) rồi [`SliceDataKeys(key2, cipher, isServer)`](DataChannel/OpenVpnKeyMethod2.cs) cắt theo `KeySizeBytes` (16/128, 32/256). `OpenVpnDataChannel` tự chọn `AesGcmCipher` theo độ dài khóa. **CHACHA20-POLY1305** chờ AEAD của **F.4**.
...
| NCP + compression framing | `OpenVpnDataCipher`/`OpenVpnPeerInfo`/`OpenVpnCompression` | IV_CIPHERS peer-info, server chốt `cipher` ở PUSH_REPLY; comp-lzo/stub-v2 framing (no-compress) |
