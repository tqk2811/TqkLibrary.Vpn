...
├─ Config/
│  ├─ WireGuardConfig.cs             (V3.f) config tĩnh (keys/address/DNS/MTU) single-peer (trường tiện ích) hoặc multi-peer (Peers) + EnumeratePeers() + ToTunnelConfig()
│  └─ WireGuardPeer.cs               (V.3) một [Peer]: PublicKey/PresharedKey?/AllowedIps (v4+v6)/PersistentKeepaliveSeconds/Endpoint?
├─ Routing/                          (V.3) crypto-routing đa-peer
│  ├─ IpPrefix.cs                    value-object CIDR thuần: TryParse (net/prefix hoặc host) + Contains (so bit theo prefix, mask host-bits) + Equals
│  ├─ IpPacketDestination.cs         đọc dest address từ goi IP v4 (offset 16) / v6 (offset 24) — tối thiểu, không phụ thuộc IpStack
│  └─ WireGuardCryptoRouter.cs       bảng route bất biến: Build(per-peer allowed-ips) + TryRoute(dest) longest-prefix → peer index (tie = peer trước)
├─ Transport/
│  ├─ WireGuardChannel.cs            (V3.f) IPacketChannel (Medium=Ip, header 0) bọc một WireGuardTransport: Seal/Deliver/SendKeepalive + callback timer
│  └─ WireGuardMultiPeerChannel.cs   (V.3) IPacketChannel ổn định trước N WireGuardChannel: route outbound theo dest, gom inbound type-4 demux receiver-index; SetPeerChannel swap 1 peer khi rekey
...
| `WireGuardConfig` | (V3.f) config tĩnh: `PrivateKey` + `Address`/`AddressV6`/`PrefixLength*` + `DnsServers` + `Mtu`(1420); **single-peer** qua `PeerPublicKey?`/`PresharedKey?`/`AllowedIps` (mặc định `0.0.0.0/0,::/0`)/`PersistentKeepaliveSeconds`, **hoặc multi-peer** qua `Peers` (`IReadOnlyList<WireGuardPeer>`); `EnumeratePeers()` chuẩn hóa (Peers nếu có, ngược lại 1 peer từ trường tiện ích — không có cả hai ⇒ ném); `ToTunnelConfig()` → `TunnelConfig` tĩnh (routes = hợp allowed-ips mọi peer) | [Config/WireGuardConfig.cs:20](Config/WireGuardConfig.cs#L20) |
| `WireGuardPeer` | (V.3) một `[Peer]`: `PublicKey` + `PresharedKey?` + `AllowedIps` (CIDR v4+v6, mặc định `0.0.0.0/0,::/0`) + `PersistentKeepaliveSeconds` + `Endpoint?` (per-peer, null ⇒ dùng endpoint connection) | [Config/WireGuardPeer.cs:12](Config/WireGuardPeer.cs#L12) |
| `IpPrefix` | (V.3) value-object CIDR bất biến (struct): `TryParse` (`net/prefix` hoặc host ⇒ /32 hoặc /128, mask host-bits, lệch family ⇒ false), `Contains(IPAddress)` so `PrefixLength` bit đầu, `PrefixLength` = độ đặc hiệu longest-prefix; `Equals`/`GetHashCode`/`ToString` | [Routing/IpPrefix.cs:18](Routing/IpPrefix.cs#L18) |
| `IpPacketDestination` | (V.3) helper thuần đọc dest từ goi IP: `TryGetDestination` (v4 offset 16 / v6 offset 24 theo version nibble, ngắn/không-IP ⇒ false) + `FamilyOf`; cố ý tối thiểu để không kéo phụ thuộc IpStack vào project protocol | [Routing/IpPacketDestination.cs:13](Routing/IpPacketDestination.cs#L13) |
| `WireGuardCryptoRouter` | (V.3) bảng route longest-prefix bất biến: `Build(per-peer allowed-ips)` (skip CIDR sai), `TryRoute(dest, out peerIndex)` chọn prefix dài nhất phủ dest (tie ⇒ peer thêm trước; không phủ ⇒ false), `Count` | [Routing/WireGuardCryptoRouter.cs:17](Routing/WireGuardCryptoRouter.cs#L17) |
| `WireGuardChannel` | (V3.f) `IPacketChannel` (Medium=Ip, MaxHeaderLength=0) bọc một `WireGuardTransport`: `WriteIpPacketAsync`=Seal+send, `Deliver`=TryOpen→`InboundIpPacket` (drop payload rỗng=keepalive), `SendKeepaliveAsync`; callback `onPacketSealed`/`onPacketReceived` cho driver bơm timer; một channel = một thế hệ key của một peer | [Transport/WireGuardChannel.cs:24](Transport/WireGuardChannel.cs#L24) |
| `WireGuardMultiPeerChannel` | (V.3) `IPacketChannel` ổn định trước N `WireGuardChannel` + `WireGuardCryptoRouter`: `WriteIpPacketAsync` route dest → peer (single-peer ⇒ peer 0 không cần parse), `SetPeerChannel(i, channel)` cài/hot-swap channel 1 peer (rekey, các peer khác giữ nguyên), `Deliver(span)` thử từng peer channel → trả index peer mở được (hoặc −1), forward inbound chung; `onNoRoute` báo drop | [Transport/WireGuardMultiPeerChannel.cs:25](Transport/WireGuardMultiPeerChannel.cs#L25) |
| `WireGuardTai64n` | encode TAI64N: `Now()`/`Encode(instant)` ra 12B (giây TAI base `0x400000000000000A` BE + ns BE), `Compare` lexicographic đơn điệu | [Handshake/WireGuardTai64n.cs:18](Handshake/WireGuardTai64n.cs#L18) |
...
| `WireGuardInitiationMessage` | record gói type-1 đã decode: `SenderIndex`/`UnencryptedEphemeral`/`EncryptedStatic`/`EncryptedTimestamp`/`Mac1`/`Mac2` | [Handshake/Models/WireGuardInitiationMessage.cs:9](Handshake/Models/WireGuardInitiationMessage.cs#L9) |
| `WireGuardResponseMessage` | record gói type-2 đã decode: `SenderIndex`/`ReceiverIndex`/`UnencryptedEphemeral`/`EncryptedNothing`/`Mac1`/`Mac2` | [Handshake/Models/WireGuardResponseMessage.cs:8](Handshake/Models/WireGuardResponseMessage.cs#L8) |
...
| `WireGuardKeyPair` | record cặp khóa X25519 `PrivateKey`(32)/`PublicKey`(32) | [Handshake/Models/WireGuardKeyPair.cs:8](Handshake/Models/WireGuardKeyPair.cs#L8) |
| `WireGuardTransportKeys` | record cặp transport key cuối handshake `SendKey`/`ReceiveKey` (32B); chéo nhau giữa 2 peer | [Handshake/Models/WireGuardTransportKeys.cs:10](Handshake/Models/WireGuardTransportKeys.cs#L10) |
...
- **Initiation** ([`CreateInitiation`](Handshake/WireGuardHandshake.cs#L112) ↔ [`ConsumeInitiation`](Handshake/WireGuardHandshake.cs#L149)): `InitializeWireGuard` (seed ck/h) → `MixHash(Sresp^pub)` → gen ephemeral, `MixKey(Ei^pub)`+`MixHash(Ei^pub)` → `MixKey(DH(Ei,Sresp))`+`EncryptAndHash(Sinit^pub)` → `MixKey(DH(Sinit,Sresp))`+`EncryptAndHash(TAI64N)`. Responder mở ngược lại, recover được static public + timestamp của initiator (sai tag ⇒ trả false).
- **Response** ([`CreateResponse`](Handshake/WireGuardHandshake.cs#L189) ↔ [`ConsumeResponse`](Handshake/WireGuardHandshake.cs#L222)): gen ephemeral, `MixKey(Er^pub)`+`MixHash(Er^pub)` → `MixKey(DH(Er,Ei))` → `MixKey(DH(Er,Si))` → `MixKeyAndHash(PSK)` → `EncryptAndHash(ε)` (gói rỗng, chỉ tag). Initiator mở `enc_empty` (sai PSK/tag ⇒ false).
...
Sau handshake, mỗi phía dựng một [`WireGuardTransport`](DataChannel/WireGuardTransport.cs#L24) trên cặp [`WireGuardTransportKeys`](Handshake/Models/WireGuardTransportKeys.cs#L10) từ `Split` (send-key bên này = receive-key bên kia). State machine đối xứng thuần, không socket:
...
3. **Anti-replay u64** ([`WireGuardReplayProtector`](DataChannel/WireGuardReplayProtector.cs#L19)): vì [`AntiReplayWindow`](../TqkLibrary.VpnClient.Crypto/AntiReplayWindow.cs#L8) (dùng chung ESP/OpenVPN) chỉ nhận `uint` và đánh số từ 1, protector ánh xạ counter-0-based → window-1-based (`low+1`) trong một **epoch** high-32 và tự theo dõi high-32: epoch tiến (counter vượt 2³²) ⇒ reset window mới; epoch cũ ⇒ luôn loại như replay. Thực tế WG rekey trước `2⁶⁰` gói nên epoch hầu như không đổi, nhưng vẫn đúng spec u64.

## Luồng crypto-routing đa-peer (V.3)

WireGuard chọn peer (và do đó khóa) cho mỗi gói **outbound** bằng longest-prefix match trên địa chỉ đích — không có IPCP/route OS, bảng route nằm trong data plane userspace:

1. **Build bảng route**: [`WireGuardCryptoRouter.Build`](Routing/WireGuardCryptoRouter.cs#L32) nhận danh sách allowed-ips theo peer (peer i → CIDR list), parse từng CIDR thành [`IpPrefix`](Routing/IpPrefix.cs#L18) (skip cái sai), giữ thứ tự peer.
2. **Gửi** ([`WireGuardMultiPeerChannel.WriteIpPacketAsync`](Transport/WireGuardMultiPeerChannel.cs#L86)): [`IpPacketDestination.TryGetDestination`](Routing/IpPacketDestination.cs#L24) đọc dest từ goi IP → [`TryRoute`](Routing/WireGuardCryptoRouter.cs#L52) chọn prefix **dài nhất** phủ dest → seal qua `WireGuardChannel` của peer đó. **Tie** độ dài prefix ⇒ peer thêm trước thắng (giống "first matching most-specific" của WireGuard). **Không peer nào phủ** ⇒ drop (báo `onNoRoute`, driver log `VpnDropReason.NoRoute`). **Single-peer** (point-to-point, 1 peer `0.0.0.0/0,::/0`) ⇒ mọi gói về peer 0, không bắt buộc parse IP header.
3. **Nhận** ([`Deliver`](Transport/WireGuardMultiPeerChannel.cs#L114)): gói type-4 mang receiver-index của session đích nên thử lần lượt từng peer channel `Deliver` (kiểm index của mình + tag) đến khi một peer mở được → trả index peer (gói inbound từ peer nào gán về peer đó); không peer nào mở ⇒ −1 (foreign/forged/replayed).
4. **Rekey**: [`SetPeerChannel(i, channel)`](Transport/WireGuardMultiPeerChannel.cs#L72) hot-swap channel của **một** peer (make-before-break per-peer) — các peer khác giữ nguyên, facade trước channel không rebind.
...
| Noise Protocol Framework | `NoiseSymmetricState` (V3.a) | giao thức `Noise_IKpsk2_25519_ChaCha20Poly1305_BLAKE2s`; **seed string** = `Noise_IKpsk2_25519_ChaChaPoly_BLAKE2s` (viết tắt cipher — đúng `wg` thật) |
...
- `WireGuardConstants.Construction` re-export `NoiseSymmetricState.Construction` = **`Noise_IKpsk2_25519_ChaChaPoly_BLAKE2s`** (cipher **viết tắt** `ChaChaPoly`, KHÔNG phải tên Noise đầy đủ `ChaCha20Poly1305`) — đây chính là chuỗi reference WireGuard (wireguard-go `NoiseConstruction` / kernel) hash để seed `ck0`/`h0`. **BUG đã sửa qua validate live (V.3)**: trước đây dùng tên đầy đủ ⇒ `ck0`/`h0` lệch ⇒ `wg` thật từ chối initiation (`Received invalid initiation message`: AAD transcript-hash sai làm AEAD msg.static fail); hằng sai vẫn tự-interop (initiator↔responder cùng `ck0` sai) nên test offline self-pair PASS — chỉ live mới lộ. Đúng giá trị: `ck0=60e26dae…`, `h0=2211b361…` (InitialChainKey/InitialHash chính chủ).
- **Interop `wg` thật đã VALIDATE LIVE (V.3, lab [`wireguard`](../../lab/wireguard) — wireguard-go userspace)**: handshake Noise_IKpsk2 (+PSK) hoàn tất, server `wg show` thấy `latest handshake` + `transfer received/sent > 0`, data 2 chiều (TCP round-trip qua tunnel). Đây là lần phát hiện + sửa bug construction-string ở trên.
