...
- [ ] **P1.1 — IPv6 trong tunnel (IPV6CP, RFC 5072)** — bật IPv6 end-to-end qua SSTP/L2TP. IP stack **đã dual-stack**. **Đã xong (1) IPV6CP negotiator link-local** ([`Ipv6cpNegotiator`](../src/TqkLibrary.Vpn.Ppp/Ipv6cpNegotiator.cs#L14) thương lượng Interface-Identifier → `fe80::/64`) + wire opt-in vào [`PppEngine(enableIpv6)`](../src/TqkLibrary.Vpn.Ppp/PppEngine.cs#L33) (event `Ipv6Up`/`AssignedAddressV6`, song song IPCP, không đổi link-up IPv4) + test offline (as-built [`10`](10-codebase-architecture-and-flow.md) §5/§8/§9). **Còn lại**: (2) địa chỉ **global** qua SLAAC-từ-RA trên link hoặc DHCPv6 (tùy server — tái dùng codec NDISC khi có **L2.4**); (3) đẩy `AssignedAddressV6` lên [`TunnelConfig`](../src/TqkLibrary.Vpn.Abstractions/Drivers/Models/TunnelConfig.cs#L12) + bật `enableIpv6` ở 2 driver, dựng stack qua overload dual-stack [`TcpIpStack(channel, v4, v6)` @ :49](../src/TqkLibrary.Vpn.IpStack/TcpIpStack.cs#L49) ở **cả 2 driver**; (4) demo bật `IsSupportIpv6=true` + bỏ chặn đích IPv6 ở `VpnConnectSource`/`VpnUdpAssociateSource`. **Rủi ro**: đa số server VPN Gate chỉ cấp IPv4 ⇒ test live cần lab (**Q.1**, SoftEther/RRAS bật IPv6 pool).
- [ ] **P1.2 — Outer IPv6 (kết nối tới server qua IPv6)**. **Đã xong (code + test offline)**: seam resolve dùng chung [`IHostResolver`/`DnsHostResolver`](../src/TqkLibrary.Vpn.Abstractions/Net/DnsHostResolver.cs#L14) chọn họ địa chỉ theo [`AddressFamilyPreference`](../src/TqkLibrary.Vpn.Abstractions/Net/AddressFamilyPreference.cs) (ưu tiên config, fallback họ còn lại; `Auto`=IPv4-first ⇒ không đổi hành vi live cũ) trên [`VpnEndpoint`](../src/TqkLibrary.Vpn.Abstractions/Drivers/Models/VpnEndpoint.cs#L4); SSTP [`TlsByteStream`](../src/TqkLibrary.Vpn.Drivers.Sstp/Transport/TlsByteStream.cs) connect `TcpClient(address.AddressFamily)` theo `IPAddress` (giữ SNI host); L2TP/IPsec bỏ filter cứng IPv4 ở `ResolveAsync` + [`NatTraversalChannel`](../src/TqkLibrary.Vpn.Ipsec/Nat/NatTraversalChannel.cs#L20) bind UDP theo `remoteAddress.AddressFamily` (checksum UDP v6 đã có pseudo-header). **Còn lại (cần lab Q.1 dual-stack)**: kiểm chứng **live** IKE NAT-D + ESP encap trên UDP/IPv6 và SSTP-over-IPv6 thật; xác nhận resolve theo AAAA thực tế. Test live qua lab dual-stack.
- [ ] **P1.3 — Rekey Phase 1 in-place (IKEv1)**. **Đã xong (code + test offline)**: timer ~90% Phase 1 (8h) vào [`RekeyPhase1Async`](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L555) — dựng **ISAKMP SA mới hoàn toàn** (Main Mode đầy đủ, cookie/DH/SKEYID\* mới) trên cùng kênh UDP/4500 đã float, Quick Mode dưới SA mới → ESP CHILD SA mới, [`SwapSession`](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/IpsecL2tpTransport.cs#L54) make-before-break (tái dùng y hệt rekey Phase 2), Delete ISAKMP SA cũ, swing `_ike` sang SA mới; reply rekey tách khỏi DPD SA cũ trên cùng socket nhờ cookie SA mới ([`IkeV1Client.IsForThisSa`](../src/TqkLibrary.Vpn.Ipsec/Ike/V1/IkeV1Client.cs#L18) → `_phase1RekeyWaiter`); chung guard `_rekeyInProgress` với rekey Phase 2; rekey lỗi ⇒ giữ SA cũ + retry 2 phút trong biên trước hạn cứng. Test offline [`IkeV1HandshakeTests`](../tests/TqkLibrary.Vpn.Ipsec.Ike.Tests/IkeV1HandshakeTests.cs) (SA mới mang traffic 2 chiều khi SA cũ còn giải mã được gói in-flight; `IsForThisSa` khớp/loại cookie). **Còn lại (cần lab Q.1)**: kiểm chứng **live** với gateway đặt lifetime Phase 1 ngắn — không rớt tunnel suốt rekey; xác nhận server chấp nhận Main Mode rekey trên cổng 4500 đã float.
- [ ] **P1.7 — Multi-session per tunnel (L2TP)**. **Đã xong (code + test offline)**: `L2tpClient` tách thành tunnel-level + N [`L2tpSession`](../src/TqkLibrary.Vpn.L2tp/L2tpSession.cs#L9) (mỗi session id riêng, `DataReceived`/`SendDataAsync`/CDN riêng), inbound data **demux theo session id**, ICRP/CDN tương quan theo session id ở header (fallback session đang mở khi peer trả 0); [`L2tpClient.OpenSessionAsync`](../src/TqkLibrary.Vpn.L2tp/L2tpClient.cs#L80) mở session phụ (ICRQ/ICRP/ICCN) trên cùng tunnel/control-channel; driver [`L2tpIpsecConnection.OpenAdditionalSessionAsync`](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecConnection.cs#L138) chạy `PppEngine`/IPCP riêng cho session phụ → địa chỉ/kênh thứ hai, [`L2tpIpsecVpnConnection.OpenSessionAsync`](../src/TqkLibrary.Vpn.Drivers.L2tpIpsec/L2tpIpsecVpnConnection.cs#L32) bọc thành `IVpnSession` thêm (thay `NotSupportedException` cũ). Test offline [`L2tpClientMultiSessionTests`](../tests/TqkLibrary.Vpn.L2tp.Tests/L2tpClientMultiSessionTests.cs) (session #2 id phân biệt + demux data đúng session; server CDN từ chối ICRQ #2 ⇒ ném). **Best-effort theo server** (đa số remote-access server chỉ cho 1 session; design `03`); session phụ **không** sống qua auto-reconnect (cleanup xóa `_extraSessions`). SSTP **strictly 1:1** (header không có session ID — không mux được, giữ `NotSupportedException` + docs). **Còn lại (cần lab Q.1)**: kiểm chứng **live** với server cho phép >1 session (SoftEther/RRAS cấu hình multi-call) — xác nhận 2 PPP/IPCP độc lập chạy đồng thời trên một IKE/IPsec SA.
