...
- **Giới hạn (kế thừa từ IpStack):** `TcpConnection` là active-open (chỉ client, **không** listen/accept). Stack **tin cậy đầy đủ** — retransmit/RTO + flow control + out-of-order reassembly + half-close FSM (vì peer là host thật trên internet, đoạn gateway↔host mất gói/đảo thứ tự thật) — nhưng cố ý **không** SACK/congestion-control ([TcpConnection.cs:12-24](../TqkLibrary.Vpn.IpStack/Tcp/TcpConnection.cs#L12-L24)); MSS suy từ link MTU (`Mtu`−40, mặc định 1360, clamp theo MSS peer).
