...
- **VALIDATE LIVE — GRE/IPIP/SIT cả 3 kiểu 2 CHIỀU XONG (2026-06-24)** — lab [`lab/ipencap`](../../lab/ipencap/README-vi.md) (peer Linux `ip tunnel` thuần tự host Docker, 2 container bridge no-NAT subnet tĩnh; client demo `gre://`|`ipip://`|`sit://` với IP tunnel TĨNH `?addr`/`?peer` vì connectionless không có IPCP). **GRE proto-47**: pcap `GREv0 Flags [none]` (no key/seq/checksum — khớp Linux gre no-key byte-for-byte) chở ICMP echo→reply + UDP DNS 2 chiều, gateway RTT 1ms; **IPIP proto-4**: pcap `proto IPIP (4)` header-less inner-IP verbatim, ICMP+UDP 2 chiều; **SIT proto-41**: pcap `proto IPv6 (41)` chở IPv6 inner verbatim (`fd00::2↔fd00::1`) UDP 2 chiều, stack dual-stack. **0 bug — codec IpEncap đúng byte-for-byte với Linux kernel ip-tunnel ngay lần đầu** (encap trần connectionless, không handshake/auth nên sạch nhất). KHÔNG sửa code driver/protocol.
- **Validate live còn lại** — reconnect chỉ kích khi caller báo link-loss tường minh (encap connectionless không tự phát hiện); GRE Key/Sequence/Checksum (RFC 2890 — lab dùng GRE no-key) chưa kiểm live với gateway có key.
