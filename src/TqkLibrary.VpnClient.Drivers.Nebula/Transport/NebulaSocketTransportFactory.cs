...
{
    /// <summary>
    /// The production <see cref="INebulaTransportFactory"/>: opens a real connected UDP socket to the Nebula peer.
    /// Nebula is UDP-only — one datagram is one packet, no framing — so the transport is a thin
    /// <see cref="IDatagramTransport"/> plus a receive loop that raises each datagram to the connection's handler. The
    /// socket I/O is exercised live (lab); the offline tests drive the connection through an in-process factory.
    /// Mirrors <c>WireGuardSocketTransportFactory</c>'s <c>UdpDatagramSocket</c>.
    /// </summary>
    public sealed class NebulaSocketTransportFactory : INebulaTransportFactory
    {
        ...
        public NebulaSocketTransportFactory(int receiveBufferSize = 65535)
        {
        ...
        /// <inheritdoc/>
        public async Task<NebulaTransportHandle> ConnectAsync(IPEndPoint remote, CancellationToken cancellationToken)
        {
            ...
            return new NebulaTransportHandle(socket, socket.SetReceiver, socket.RunReceiveLoopAsync);
        }

        /// <summary>A connected UDP datagram pipe over a real socket (live-only; mirrors the WireGuard driver's UDP socket).</summary>
