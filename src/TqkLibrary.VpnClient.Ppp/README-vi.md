...
- **Phụ thuộc (ProjectReference):**
  ...
  - [`TqkLibrary.VpnClient.Ethernet`](../TqkLibrary.VpnClient.Ethernet) — tái dùng engine L3 NDISC/SLAAC/DHCPv6 (`NdiscResolver`/`Ipv6AddressConfigurator`) cho **global IPv6 over PPP** (`Ipv6/`, P1.1); Ethernet chỉ ref Abstractions nên reference này **acyclic** — [TqkLibrary.VpnClient.Ppp.csproj:12](TqkLibrary.VpnClient.Ppp.csproj#L12).
...
├─ Ipv6/                      # Global IPv6 over PPP (P1.1): xin địa chỉ global sau IPV6CP link-local
│  ├─ IPppIpv6Autoconfigurator.cs   # Hợp đồng: RS/RA-SLAAC/DHCPv6 trên kênh PPP → TunnelConfig global (null nếu không RA)
│  ├─ PppIpv6Autoconfigurator.cs    # Impl: tái dùng NdiscResolver/Ipv6AddressConfigurator (Ethernet) qua adapter; IID = IPV6CP
│  └─ PppEthernetChannelAdapter.cs  # IPacketChannel L3 ⇄ IEthernetChannel (bỏ/ghép 14B Ethernet header; MAC tổng hợp cosmetic)
├─ Framing/
...
| `IPppIpv6Autoconfigurator` | Hợp đồng lấy địa chỉ IPv6 **global** sau IPV6CP (P1.1): `TryConfigureAsync(channel, linkLocal, interfaceId, ct)` → `TunnelConfig?` (global + prefix + DNS v6 + route `::/0`), `null` nếu không router | [Ipv6/IPppIpv6Autoconfigurator.cs:15](Ipv6/IPppIpv6Autoconfigurator.cs#L15) |
| `PppIpv6Autoconfigurator` | Impl mặc định: gửi RS, nhận RA-SLAAC/DHCPv6 trên kênh PPP, tái dùng `NdiscResolver`/`Ipv6AddressConfigurator` (Ethernet) qua adapter; IID = IPV6CP (`InterfaceIdentifierOverride`); best-effort, timeout ngắn (ctor nhận timeout cho test) | [Ipv6/PppIpv6Autoconfigurator.cs:20](Ipv6/PppIpv6Autoconfigurator.cs#L20) |
| `PppEthernetChannelAdapter` | Trình diễn `IPacketChannel` L3 (PPP) thành `IEthernetChannel` để tái dùng engine NDISC/SLAAC/DHCPv6 (vốn viết cho Ethernet): gửi bỏ 14B Ethernet header → `WriteIpPacketAsync`; nhận IPv6 thuần → raise `InboundFrame`; MAC tổng hợp cosmetic; `Start()` sau khi wire handler | [Ipv6/PppEthernetChannelAdapter.cs:26](Ipv6/PppEthernetChannelAdapter.cs#L26) |
| `PppControlCodec` | Encode/decode gói control (Code/Id/Length) và TLV option | [PppControlCodec.cs:9](PppControlCodec.cs#L9) |
| `MsChapV2Authenticator` | Authenticator client trên CHAP: nhận Challenge → trả Response 49 byte, báo Success/Failure, giữ NtResponse cho HLAK; codec mật mã ủy thác cho [`MsChapV2`](../TqkLibrary.VpnClient.Crypto/MsChapV2.cs) ở Crypto | [Auth/MsChapV2Authenticator.cs:13](Auth/MsChapV2Authenticator.cs#L13) |
...
| **RFC 5072** (IPV6CP) | `Ipv6cpNegotiator`, `Ipv6cpOptionType` | [Ipv6cpNegotiator.cs:13](Ipv6cpNegotiator.cs#L13), [Enums/Ipv6cpOptionType.cs:3](Enums/Ipv6cpOptionType.cs#L3) | Interface-Identifier §4.1 → link-local fe80::/64; Compression §4.2 reject |
...
| **[MS-CHAP]** (Microsoft, response value 49 byte) | `MsChapV2Authenticator.BuildResponse` | [Auth/MsChapV2Authenticator.cs:87-91](Auth/MsChapV2Authenticator.cs#L87-L91) | (suy luận) PeerChallenge(16)+Reserved(8)+NT-Response(24)+Flags(1) |

> Lưu ý: MS-CHAPv2 là cơ chế yếu, được dùng ở đây chỉ vì L2TP/IPsec và SSTP bắt buộc nó — codec mật mã thuần (NtPasswordHash/ChallengeHash/GenerateNTResponse) nay ở [`MsChapV2`](../TqkLibrary.VpnClient.Crypto/MsChapV2.cs) thuộc Crypto; lớp này chỉ làm khung CHAP — xem ghi chú tại [Auth/MsChapV2Authenticator.cs:9](Auth/MsChapV2Authenticator.cs#L9).

## API / cách dùng

Các điểm vào public chính:

- `PppEngine(IPppFrameChannel channel, uint magic, IPAddress localAddress, IPAddress? assignPeerAddress = null, IPAddress? assignPeerDns = null, IPppAuthenticator? authenticator = null, int mtu = 1400, bool enableIpv6 = false, byte[]? interfaceId = null, byte[]? assignPeerInterfaceId = null)` — [PppEngine.cs:33](PppEngine.cs#L33). `enableIpv6` bật IPV6CP (mặc định tắt → không đổi hành vi IPv4).
- `PppEngine.Start()` — bắt đầu thương lượng (gửi LCP Configure-Request) — [PppEngine.cs:94](PppEngine.cs#L94).
...
5. **Xác thực MS-CHAPv2.** Khung CHAP vào → `HandleAuth` → `authenticator.Handle(...)`; với Challenge thì `BuildResponse` tạo PeerChallenge ngẫu nhiên, `GenerateNTResponse` (challenge hash SHA-1 → NT hash MD4 → 3×DES) và đóng gói Response 49 byte; Success → `StartNetworkLayer()`, Failure → `AuthFailed` — [PppEngine.cs:127-144](PppEngine.cs#L127-L144), [Auth/MsChapV2Authenticator.cs:50](Auth/MsChapV2Authenticator.cs#L50), [Auth/MsChapV2Authenticator.cs:72](Auth/MsChapV2Authenticator.cs#L72), [`MsChapV2.GenerateNTResponse`](../TqkLibrary.VpnClient.Crypto/MsChapV2.cs) (Crypto).
...
7. **IPV6CP (nếu `enableIpv6`).** Song song IPCP: client xin Interface-Identifier; server gán qua Configure-Nak, `OnNak` cập nhật rồi gửi lại; `Opened` → `OnIpv6cpOpened` đặt `IsIpv6Up` + raise `Ipv6Up`, lộ `AssignedAddressV6` = fe80::/64 + IID — [Ipv6cpNegotiator.cs:13](Ipv6cpNegotiator.cs#L13), [PppEngine.cs:121-125](PppEngine.cs#L121-L125).
...
Data plane sau khi up (một `IPacketChannel` mang cả hai họ): ghi IP → `SendIpAsync` chọn Protocol theo version nibble — `0x0021` (IPv4) hoặc `0x0057` (IPv6) — [PppEngine.cs:175](PppEngine.cs#L175); IP vào (cả `Ip` lẫn `Ipv6`) → `RaiseInbound` → sự kiện `InboundIpPacket` — [PppEngine.cs:166](PppEngine.cs#L166), [PppPacketChannel.cs:39](PppPacketChannel.cs#L39). Tầng trên (`TcpIpStack` dual-stack) demux lại theo version nibble.

**Global IPv6 over PPP (P1.1, namespace `Ipv6/`).** IPV6CP chỉ cấp **link-local** `fe80::/64`; địa chỉ **global** đến từ server quảng bá prefix trên link — y hệt host trên mọi medium khác. Driver (SSTP/L2TP) gọi [`PppIpv6Autoconfigurator.TryConfigureAsync`](Ipv6/PppIpv6Autoconfigurator.cs#L44) trên `PppEngine.PacketChannel` sau khi IPV6CP up:

1. **Adapter L3→L2.** Dựng [`PppEthernetChannelAdapter`](Ipv6/PppEthernetChannelAdapter.cs#L26) trình diễn kênh PPP L3 thành `IEthernetChannel` (gửi: bỏ 14B Ethernet header → `WriteIpPacketAsync` @ [:66](Ipv6/PppEthernetChannelAdapter.cs#L66); nhận: `InboundIpPacket` → raise `InboundFrame` @ [:74](Ipv6/PppEthernetChannelAdapter.cs#L74)) — để **tái dùng nguyên** engine NDISC/SLAAC/DHCPv6 của `Ethernet` (vốn viết cho Ethernet port). MAC tổng hợp từ IID (`SynthesizeMac` @ [:86](Ipv6/PppIpv6Autoconfigurator.cs#L86)) chỉ cosmetic (Source-LLA của RS + DUID DHCPv6) trên link point-to-point.
2. **Engine SLAAC/DHCPv6.** Cắm `NdiscResolver` + `Ipv6AddressConfigurator` (option `interfaceIdentifierOverride = IID-IPV6CP`) lên adapter, wire `InboundFrame` rồi `Start()` ([PppIpv6Autoconfigurator.cs:49-64](Ipv6/PppIpv6Autoconfigurator.cs#L49-L64)); `ConfigureAsync` gửi **Router Solicitation**, nhận **RA (SLAAC)** hoặc **DHCPv6** → địa chỉ **global = prefix ‖ IID-IPV6CP** (SLAAC ưu tiên `InterfaceIdentifierOverride` thay vì EUI-64 từ MAC tổng hợp).
3. **Best-effort.** Không router/RA ⇒ `InvalidOperationException` bắt lại → trả `null` ([PppIpv6Autoconfigurator.cs:69-74](Ipv6/PppIpv6Autoconfigurator.cs#L69-L74)) (caller giữ link-local). Trả về `TunnelConfig` global + prefix length + DNS v6 + route `::/0`; `cancellationToken` (teardown) propagate `OperationCanceledException`.
...
- **IPV6CP (RFC 5072) — opt-in, chưa test live:** `Ipv6cpNegotiator` thương lượng Interface-Identifier → link-local `fe80::/64`, bật qua `PppEngine(enableIpv6: true)`; chạy song song IPCP, không đổi đường IPv4 (mặc định tắt). Data plane đã **multiplex hai họ** trên một `IPacketChannel` (send chọn 0x0021/0x0057 theo version nibble, inbound `Ipv6` đổ chung kênh) — phủ test offline qua loopback (gói IPv6 đi/về đúng Protocol 0x0057).
- **Global IPv6 over PPP (P1.1, namespace `Ipv6/`) — code + test offline:** sau IPV6CP, [`PppIpv6Autoconfigurator`](Ipv6/PppIpv6Autoconfigurator.cs#L20) xin địa chỉ **global** = prefix ‖ IID-IPV6CP (RS → RA-SLAAC / DHCPv6 trên kênh PPP) bằng cách **tái dùng nguyên** engine NDISC/SLAAC/DHCPv6 của `Ethernet` qua [`PppEthernetChannelAdapter`](Ipv6/PppEthernetChannelAdapter.cs#L26) (+ option `Ipv6AddressConfiguratorOptions.InterfaceIdentifierOverride`); best-effort (không RA ⇒ `null`, giữ link-local). Driver SSTP/L2TP gọi rồi merge vào `TunnelConfig` qua `Drivers.Core/TunnelConfigIpv6` — xem README từng driver. Test offline [`PppIpv6AutoconfiguratorTests`](../../tests/TqkLibrary.VpnClient.Ppp.Tests/PppIpv6AutoconfiguratorTests.cs) (RA→global đúng IID / không-router→null / input xấu→null). **Còn lại** (roadmap P1.1): validate live (server cấp RA-prefix IPv6, lab Q.1).
