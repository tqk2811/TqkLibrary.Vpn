...
├─ OpenConnectConnection.cs                 Điều phối (kế thừa ReconnectingVpnConnection<…> F.6): TLS stream → auth → CONNECT → bind CstpChannel → (thử DTLS swap) → receive loop + DPD/keepalive timer; supervisor/reconnect ở base
...
| `OpenConnectConnection` | Bộ điều phối — kế thừa [`ReconnectingVpnConnection<OpenConnectConnectionState>`](../TqkLibrary.VpnClient.Drivers.Core/ReconnectingVpnConnection.cs#L25) (supervisor F.6): override `EstablishAsync` (resolve → `IOpenConnectTransportFactory.ConnectAsync` (TLS) → **HTTPS config-auth** init→form→reply điền username/password lấy cookie → **HTTP CONNECT** advertise DTLS khi có UDP factory → `ParseConnectResponse` X-CSTP-*/X-DTLS-* → bind `CstpChannel` qua `Facade` → `MarkRunning` → **`TryEstablishDtlsAsync`** (server `HasDtls` + UDP factory ⇒ UDP→`DtlsDatagramTransport`→`CstpDatagramChannel` swap, fail/timeout ⇒ giữ TLS) → `MarkConnected`) + `CleanupAttemptResourcesAsync`/`StopAttemptLoop`/`OnReconnected`/`DisconnectAsync` (DISCONNECT trước teardown base); receive loop(s) + DPD/keepalive timer bơm `CstpDpdState` trên kênh active; `IsDtlsDataPlane` cờ; DPD-REQ→DPD-RESP, peer-close/DPD-dead/fault → `OnLinkLost` của base arm reconnect | [OpenConnectConnection.cs:41](OpenConnectConnection.cs#L41) |
...
| `OpenConnectReconnectOptions` | Kế thừa [`VpnReconnectOptions`](../TqkLibrary.VpnClient.Drivers.Core/Models/VpnReconnectOptions.cs#L17) (F.6) — `Enabled`/`MaxAttempts`/backoff/jitter ở base; giữ tên riêng cho public API | [OpenConnectReconnectOptions.cs:12](OpenConnectReconnectOptions.cs#L12) |
...
3. **HTTPS config-auth** ([`AuthenticateAsync`](OpenConnectConnection.cs#L270)): `OpenConnectHttpTransactor.PostAsync("/", config-auth init)` → server trả `<auth><form>` → `OpenConnectAuthCodec.TryParseForm` → điền username/password (`FillForm`) → `BuildReplyRequest` POST tiếp; lặp tới khi `IsSuccess` (`id="success"`) kèm cookie `webvpn=…` (`ExtractCookie`). Status ≠200 hoặc form-loop quá hạn ⇒ `VpnAuthenticationException`.
4. **HTTP CONNECT** ([`EstablishAsync`](OpenConnectConnection.cs#L146)): `OpenConnectConnectCodec.BuildConnectRequest(host, cookie, mtu)` → `ConnectTunnelAsync` (byte-exact, không nuốt gói CSTP đầu) → `ParseConnectResponse` map `X-CSTP-*` → `OpenConnectTunnelInfo` → `TunnelConfig` (in-band, config-push). Không có địa chỉ ⇒ `VpnServerRejectedException`.
5. **Bind data plane TLS**: `CstpChannel(stream, mtu)` + wire event (DPD-REQ→`OnDpdRequest` gửi DPD-RESP, peer-close→`OnLinkLost` của base, sent/received→`CstpDpdState`) → `Facade.SetInner(channel)`; `MarkRunning` (để peer-close trong cửa sổ DTLS-handshake bên dưới được `OnLinkLost` honour) rồi chạy `RunReceiveLoopAsync` nền.
6. **Thử DTLS (V5.c)** ([`TryEstablishDtlsAsync`](OpenConnectConnection.cs#L213)): nếu `tunnelInfo.HasDtls` (server đẩy `X-DTLS-Session-ID`+`X-DTLS-Port`) **và** có UDP factory ⇒ `IOpenConnectDatagramTransportFactory.ConnectAsync` (UDP) → `DtlsDatagramTransport.ConnectAsync` (handshake DTLS 1.2, bound timeout mặc định 10s) → `CstpDatagramChannel` + wire **cùng event** → `Facade.SetInner` (swap data plane sang DTLS, kênh TLS giữ control/fallback) → `RunReceiveLoopAsync` DTLS nền; cờ `IsDtlsDataPlane=true`. **Fail/timeout/không-offer ⇒ giữ CSTP-over-TLS** (fallback, không vỡ connect; outer-cancel mới abort). Cuối khởi timer 1s rồi `MarkConnected`.
...
- **Reconnect** (supervisor): nay nằm ở base [`ReconnectingVpnConnection`](../TqkLibrary.VpnClient.Drivers.Core/ReconnectingVpnConnection.cs#L25) (F.6) — `OnLinkLost` arm `ReconnectLoopAsync` (backoff+jitter) gọi `EstablishAsync` của driver sau `SwappablePacketChannel` ổn định; `OnReconnected` raise `Reconnected` mang `NewAddress` + `AddressChanged` (ocserv thường cấp lại cùng IP theo cookie).
