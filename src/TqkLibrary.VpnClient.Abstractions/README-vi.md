...
- **Được dùng bởi (ProjectReference trực tiếp — 20 project):** mọi driver `Drivers.*` (`Core`, `L2tpIpsec`, `Ikev2`, `Sstp`, `SoftEther`, `OpenVpn`, `OpenConnect`, `WireGuard`) + các project lõi protocol/transport (`Ipsec`, `L2tp`, `Ppp`, `Pptp`, `IpStack`, `Sockets`, `Ethernet`, `OpenConnect`, `OpenVpn`, `SoftEther`, `WireGuard`, `Transport.Dtls`). Façade `Vpn` dùng gián tiếp qua các tầng đó. (`Crypto` **không** tham chiếu Abstractions.)
...
| `DnsHostResolver` | Impl mặc định: literal-passthrough + `Dns.GetHostAddresses` + `Select` (pure static — ưu tiên 1 họ, fallback họ kia; testable không DNS). Consumer thật: SSTP `TlsByteStream`, L2TP `L2tpIpsecConnection` | [DnsHostResolver.cs:12](Net/DnsHostResolver.cs#L12) |
