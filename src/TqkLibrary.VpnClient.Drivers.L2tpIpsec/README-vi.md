...
| `L2tpIpsecNatTraversalMode` | Enum chọn cách thương lượng NAT-T: `ForcedNatT` (mặc định, ép float 4500) / `HonestFirst` (P0.8b — bind cổng 500 thật + NAT-D trung thực) | [Enums/L2tpIpsecNatTraversalMode.cs:7](Enums/L2tpIpsecNatTraversalMode.cs#L7) |
...
|-------|-------------------------|--------------------|---------|
| RFC 3706 (IKE DPD — Dead Peer Detection) | `L2tpIpsecConnection` (keepalive: L2TP HELLO + IKE DPD) | [L2tpIpsecConnection.cs:481](L2tpIpsecConnection.cs#L481) | Comment dẫn rõ `RFC 3706` |
| RFC 2759 (MS-CHAPv2) | `MsChapV2Authenticator` (dùng từ `Ppp`) | [L2tpIpsecConnection.cs:227](L2tpIpsecConnection.cs#L227) | (suy luận) — hiện thực nằm ở project `Ppp` |
| RFC 2409 (IKEv1 Main/Quick Mode, PSK) | `IkeV1Client` (từ `Ipsec`), điều phối tại đây; rekey Phase 1 in-place qua `RekeyPhase1Async` (P1.3) | [L2tpIpsecConnection.cs:269](L2tpIpsecConnection.cs#L269) | (suy luận) — hiện thực nằm ở `Ipsec/Ike/V1` |
| RFC 3947 / 3948 (IPsec NAT-T, UDP 500↔4500) | `NatTraversalChannel` (`Ipsec/Nat`); `BringUpPhase1Async` chọn forced (khai man) / honest-first (`IkeV1Client.DetectNat`) rồi `SwitchToNatTPort()` | [L2tpIpsecConnection.cs:269-318](L2tpIpsecConnection.cs#L269-L318) | (suy luận) — hiện thực ở [`Ipsec/Nat`](../TqkLibrary.VpnClient.Ipsec/Nat) |
| RFC 4303 (ESP), AES-CBC + HMAC-SHA1 / AES-GCM | `IpsecL2tpTransport`, `EspSession`; `BuildEspSession` dựng suite theo `ike.NegotiatedEsp` (CBC hoặc GCM) qua `EspSuiteSelection.BuildSuite` | [L2tpIpsecConnection.cs:214](L2tpIpsecConnection.cs#L214) | (suy luận) — cipher suite hiện thực ở `Ipsec/Esp` |
| RFC 4106 (AES-GCM trong ESP) | `EspGcmSuite` (từ `Ipsec`), chọn khi gateway negotiate AES-GCM ở Quick Mode | [L2tpIpsecConnection.cs:214](L2tpIpsecConnection.cs#L214) | (suy luận) — VPN Gate/SoftEther vẫn chọn CBC |
| RFC 2661 (L2TPv2 control/data, multi-session) | `L2tpClient` (từ `L2tp`), điều phối qua `IpsecL2tpTransport`; `OpenAdditionalSessionAsync` mở session phụ | [L2tpIpsecConnection.cs:219](L2tpIpsecConnection.cs#L219) | (suy luận) — hiện thực ở `L2tp` |
| RFC 1661 / 1332 (PPP LCP + IPCP) | `PppEngine` (từ `Ppp`) | [L2tpIpsecConnection.cs:228](L2tpIpsecConnection.cs#L228) | (suy luận) — hiện thực ở `Ppp` |
...
1. Resolve host qua `IHostResolver` theo `AddressFamilyPreference` (P1.2 — ưu tiên họ IP config, fallback họ còn lại; thay filter cứng IPv4 cũ) → `StartAttemptChannel` tạo `NatTraversalChannel` (UDP/500, **bind đúng họ địa chỉ server**) + chạy `ReceiveLoopAsync` ghép kênh IKE/ESP — [L2tpIpsecConnection.cs:322](L2tpIpsecConnection.cs#L322). Trước đó `CleanupAttemptResourcesAsync` (override hook base) dọn attempt cũ: cancel vòng đọc, null `_natt` rồi **await** `DisposeAsync` (socket cũ đóng hẳn trước khi mở socket mới) — [L2tpIpsecConnection.cs:347-376](L2tpIpsecConnection.cs#L347-L376); vòng đọc còn **guard sau mỗi `ReceiveAsync`** (re-check token + identity channel) để gói stale đang in-flight không lọt vào waiter của attempt mới — [L2tpIpsecConnection.cs:451](L2tpIpsecConnection.cs#L451).
2. **IKEv1 Phase 1 (MM1–MM4) + quyết định cổng NAT-T** trong `BringUpPhase1Async` theo `L2tpIpsecNatTraversalMode` (mặc định `ForcedNatT`) — [L2tpIpsecConnection.cs:269-318](L2tpIpsecConnection.cs#L269-L318):
...
3. **MM5/MM6** (đã mã hoá) trên cổng Phase 1 đã chốt, xác thực PSK qua `HASH_R`; sai PSK ⇒ `VpnAuthenticationException` — [L2tpIpsecConnection.cs:198-202](L2tpIpsecConnection.cs#L198-L202).
4. **Phase 2 (Quick Mode)** QM1–QM3 → khoá ESP CHILD SA + `NegotiatedEsp` (suite server chọn: AES-CBC mặc định, hoặc AES-GCM nếu gateway hỗ trợ); HASH(2) sai/không có SA ⇒ `VpnServerRejectedException` — [L2tpIpsecConnection.cs:206-213](L2tpIpsecConnection.cs#L206-L213). **Phân loại lỗi handshake (P0.8a)** ở [`ExchangeIkeAsync` :387](L2tpIpsecConnection.cs#L387): reply là NOTIFY lỗi (`IkeV1Client.TryReadRejectNotify`) ⇒ `VpnServerRejectedException` nêu mã; **no-response theo cổng** — im lặng sau float UDP/4500 ⇒ `VpnNetworkTimeoutException` chỉ rõ **nghi forced-NAT-T**, trên cổng 500 ⇒ "host unreachable/UDP blocked".
5. Dựng `EspSession` + `IpsecL2tpTransport` (data plane) — `BuildEspSession(ike.NegotiatedEsp, …)` chọn đúng cipher suite; `L2tpClient.ConnectAsync` (L2TP tunnel + session chính `PrimarySession`); `L2tpPppFrameChannel(l2tp.PrimarySession)` + `PppEngine` + `MsChapV2Authenticator` chạy LCP/Auth/IPCP (auth fail ⇒ `VpnAuthenticationException`) — [L2tpIpsecConnection.cs:214-244](L2tpIpsecConnection.cs#L214-L244).
6. Khi `LinkUp`: (nếu bật IPv6) `AwaitIpv6GraceAsync` chờ ngắn 2s cho IPV6CP mở để lộ link-local `fe80::/64` vào `TunnelConfig.AssignedAddressV6` (best-effort, không fail nếu server không có IPV6CP) → cắm kênh PPP vào facade `Facade` (base) `SwappablePacketChannel` rồi `StartKeepalive()` (gọi `MarkConnected()` base) — [L2tpIpsecConnection.cs:248-249](L2tpIpsecConnection.cs#L248-L249), [AwaitIpv6GraceAsync @ :255](L2tpIpsecConnection.cs#L255).

### Keepalive · rekey · teardown · reconnect

- **Keepalive:** L2TP HELLO (60s) + IKE DPD R-U-THERE (20s, miss ≥3 ⇒ link lost) — [L2tpIpsecConnection.cs:481-572](L2tpIpsecConnection.cs#L481-L572). `StopAttemptLoop` (override — [L2tpIpsecConnection.cs:497](L2tpIpsecConnection.cs#L497)) dispose 5 timer (hello/dpd/rekey-Phase2/Phase1/drop) + tắt `_espActive`; cờ "tunnel up" do base quản (`IsRunning`).
- **Rekey Phase 2 (make-before-break) — 2 trigger:** (a) timer ~90% lifetime (3600s); (b) **sequence-exhaustion** — sequence ESP outbound chạm high-watermark ~75%×2³² thì `RekeyNeeded` kích rekey **trước khi** wrap (RFC 4303 §3.3.3, tránh `OverflowException`). Cả hai vào `RekeyPhase2Async` (guard `_rekeyInProgress`) → Quick Mode mới → `SwapSession` (gửi trên SA mới ngay + re-arm watermark, giữ SA cũ để nhận trong grace 10s rồi `DropPreviousInbound`) — [L2tpIpsecConnection.cs:579-603](L2tpIpsecConnection.cs#L579-L603) · [EspDataPlane.cs:41-101](../TqkLibrary.VpnClient.Ipsec/Esp/EspDataPlane.cs#L41-L101) (scaffolding swap/watermark dùng chung, tách khỏi `IpsecL2tpTransport`).
- **Rekey Phase 1 in-place (make-before-break, P1.3):** timer ~90% Phase 1 (8h) vào `RekeyPhase1Async` — dựng **ISAKMP SA mới hoàn toàn** (Main Mode đầy đủ, cookie/DH/SKEYID\* mới) trên cùng kênh UDP/4500 đã float, Quick Mode dưới SA mới → ESP CHILD SA mới, `SwapSession` (tái dùng y hệt rekey Phase 2), Delete ISAKMP SA cũ, rồi swing `_ike` sang SA mới. Reply rekey tách khỏi DPD SA cũ trên cùng socket nhờ cookie SA mới (`IkeV1Client.IsForThisSa` → `_phase1RekeyWaiter`); chung guard `_rekeyInProgress` với rekey Phase 2; rekey lỗi ⇒ giữ SA cũ + retry 2 phút trong biên trước hạn cứng. Không rớt tunnel suốt rekey — [L2tpIpsecConnection.cs:638-717](L2tpIpsecConnection.cs#L638-L717).
- **Multi-session (P1.7, best-effort):** `OpenAdditionalSessionAsync` mở thêm session L2TP (`L2tpClient.OpenSessionAsync`) trên cùng IKE/IPsec SA + control channel rồi chạy một `PppEngine` riêng → địa chỉ/kênh thứ hai; teardown gửi CDN cho mọi session phụ trước CDN/StopCCN session chính. Đa số server từ chối ICRQ thứ hai bằng CDN (⇒ exception); session phụ **không** được tái lập khi auto-reconnect (cleanup xóa `_extraSessions`) — [L2tpIpsecConnection.cs:150-171](L2tpIpsecConnection.cs#L150-L171).
- **Link-loss + supervisor (F.6, gom về base):** mọi nguồn (DPD chết, server Delete, L2TP teardown) gọi `OnLinkLost` của base ([ReconnectingVpnConnection.cs:161](../TqkLibrary.VpnClient.Drivers.Core/ReconnectingVpnConnection.cs#L161)) → `ReconnectLoopAsync` ([@ :189](../TqkLibrary.VpnClient.Drivers.Core/ReconnectingVpnConnection.cs#L189)) với exponential backoff + jitter; reconnect thành công gọi `OnReconnected` (override — [L2tpIpsecConnection.cs:727](L2tpIpsecConnection.cs#L727)) raise `Reconnected` → cập nhật `TunnelConfig` qua `L2tpIpsecVpnSession.ApplyReconnect`.
- **Teardown:** `DisconnectAsync` (override — [L2tpIpsecConnection.cs:769](L2tpIpsecConnection.cs#L769)) gửi L2TP CDN + StopCCN và IKE Delete (ESP + ISAKMP) trên transport còn sống (best-effort, 2s), rồi gọi `DisconnectCoreAsync()` base (hủy reconnect + await supervisor + cleanup transport).
