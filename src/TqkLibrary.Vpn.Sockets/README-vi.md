...
- **Giới hạn (kế thừa từ IpStack):** `TcpConnection` là active-open (chỉ client, **không** listen/accept). Stack **tin cậy đầy đủ** — retransmit/RTO (RFC 6298) + flow control + zero-window persist + out-of-order reassembly + NewReno congestion-control (RFC 5681/6582) + window scaling (RFC 7323) + SACK (RFC 2018/6675) + PMTUD (RFC 1191/8201) + half-close FSM (FINWAIT/CLOSING/TIMEWAIT/CLOSEWAIT/LASTACK) (vì peer là host thật trên internet, đoạn gateway↔host mất gói/đảo thứ tự thật) ([TcpConnection.cs:13-27](../TqkLibrary.Vpn.IpStack/Tcp/TcpConnection.cs#L13-L27)); MSS suy từ link MTU (`Mtu`−40, mặc định 1360, clamp theo MSS peer + PMTUD hạ thêm khi ICMP "fragmentation needed").
